...
uses Controls, Classes, frm_connection, DB, SysUtils, utilerias, ZDataset, dialogs, Math, Forms, UnitExcepciones,
  sTrUtils, StdCtrls, DBCtrls, RXDBCtrl, global, DBGrids, Clipbrd, cxDbEdit, Menus, RxLookup, cxDBLookupComboBox,rxCurrEdit ;
...
function TipoJornada(sParamTipo, sParamFolio :string; dParamFecha :tDate; tOrigen: TComponent):string;
...
function FechaLargaCorta(sParamFechaTexto: String): string;
...
function sFnBuscaItem(sParamConvenio,sNumeroActividad, dParamWbsAnt, dParamPrimerItem, dParamTipo, dParamOrden, dParamTabla: string; iParamNivel: Byte): string;
function sfnGeneraItem(dParamConsecutivo: integer): string;
function sFnInsertaItem(sParamConvenio,sNumeroActividad, dParamWbsAnt, dParamPrimerItem, dParamTipo, dParamTabla, dParamOrden, dParamAnexo: string; iParamNivel: Byte): string;
...
begin
  ...
  begin
      ...
      if connection.uconfiguracion.FieldValues['lTiempoMuertoTurnos'] = 'Si' then
    ...
    begin
      ...
      if QryJornada.RecordCount > 0 then
        ...
        if sMyDia = 'Lunes' then
          iJornada := iJornada + connection.uconfiguracion.FieldValues['iJLunes']
        else
          if sMyDia = 'Martes' then
            iJornada := iJornada + connection.uconfiguracion.FieldValues['iJMartes']
          else
            if sMyDia = 'Miercoles' then
              iJornada := iJornada + connection.uconfiguracion.FieldValues['iJMiercoles']
            else
              if sMyDia = 'Jueves' then
                iJornada := iJornada + connection.uconfiguracion.FieldValues['iJJueves']
              else
                if sMyDia = 'Viernes' then
                  iJornada := iJornada + connection.uconfiguracion.FieldValues['iJViernes']
                else
                  if sMyDia = 'Sabado' then
                    iJornada := iJornada + connection.uconfiguracion.FieldValues['iJSabado']
                  else
                    if sMyDia = 'Domingo' then
                      iJornada := iJornada + connection.uconfiguracion.FieldValues['iJDomingo'];
...
//        QryJornada.Destroy ;
  ...
  begin
    ...
    iJornada := iJornada + (connection.uconfiguracion.FieldValues['iJLunes'] * Semanas);
    iJornada := iJornada + (connection.uconfiguracion.FieldValues['iJMartes'] * Semanas);
    iJornada := iJornada + (connection.uconfiguracion.FieldValues['iJMiercoles'] * Semanas);
    iJornada := iJornada + (connection.uconfiguracion.FieldValues['iJJueves'] * Semanas);
    iJornada := iJornada + (connection.uconfiguracion.FieldValues['iJViernes'] * Semanas);
    iJornada := iJornada + (connection.uconfiguracion.FieldValues['iJSabado'] * Semanas);
    iJornada := iJornada + (connection.uconfiguracion.FieldValues['iJDomingo'] * Semanas);
  end;
  result := iJornada;
...
begin
  ...
  if QryJornada.RecordCount > 0 then
    ...
    if sMyDia = 'Lunes' then
      iJornada := connection.uconfiguracion.FieldValues['iJLunes']
    else
      if sMyDia = 'Martes' then
        iJornada := connection.uconfiguracion.FieldValues['iJMartes']
      else
        if sMyDia = 'Miercoles' then
          iJornada := connection.uconfiguracion.FieldValues['iJMiercoles']
        else
          if sMyDia = 'Jueves' then
            iJornada := connection.uconfiguracion.FieldValues['iJJueves']
          else
            if sMyDia = 'Viernes' then
              iJornada := connection.uconfiguracion.FieldValues['iJViernes']
            else
              if sMyDia = 'Sabado' then
                iJornada := connection.uconfiguracion.FieldValues['iJSabado']
              else
                if sMyDia = 'Domingo' then
                  iJornada := connection.uconfiguracion.FieldValues['iJDomingo'];
  result := iJornada;
  QryJornada.Destroy;
end;
...
begin
    ...
    connection.ucontrato.Params.ParamByName('Contrato').Value := sParamContrato ;
    ...
    connection.uconfiguracion.Params.ParamByName('Contrato').Value := sParamContrato ;
...
begin
    ...
    if sParamTipo = 'Dia' then
      iJornada := ifnJornadaDia(global_contrato, dParamFecha, tOrigen)
    else
    begin
       zqValida.Active := False;
       zqValida.SQL.Clear;
       zqValida.SQL.Add('select iJornada from ordenesdetrabajo where sContrato =:Contrato and sNumeroOrden =:Folio');
       zqValida.ParamByName('Contrato').AsString := global_contrato;
       zqValida.ParamByName('Folio').AsString    := sParamFolio;
       zqValida.Open;

       if zqValida.RecordCount > 0 then
          iJornada := zqValida.FieldByName('iJornada').AsInteger;
    end;

    if iJornada < 10 then
      result := '0' + Trim(IntToStr(iJornada)) + ':00'
    else
      result := Trim(IntToStr(iJornada)) + ':00';
...
var
   sCadena : string;
begin
   sCadena := sCadena + copy(sParamFechaTexto,1,pos('de',sParamFechaTexto)-2) + '/';
   if pos('enero',sParamFechaTexto) > 0 then
      sCadena := sCadena +'01'+'/';
   if pos('febrero',sParamFechaTexto) > 0 then
      sCadena := sCadena +'02'+'/';
   if pos('marzo',sParamFechaTexto) > 0 then
      sCadena := sCadena +'03'+'/';
   if pos('abril',sParamFechaTexto) > 0 then
      sCadena := sCadena +'04'+'/';
   if pos('mayo',sParamFechaTexto) > 0 then
      sCadena := sCadena +'05'+'/';
   if pos('junio',sParamFechaTexto) > 0 then
      sCadena := sCadena +'06'+'/';
   if pos('julio',sParamFechaTexto) > 0 then
      sCadena := sCadena +'07'+'/';
   if pos('agosto',sParamFechaTexto) > 0 then
      sCadena := sCadena +'08'+'/';
   if pos('septiembre',sParamFechaTexto) > 0 then
      sCadena := sCadena +'09'+'/';
   if pos('octubre',sParamFechaTexto) > 0 then
      sCadena := sCadena +'10'+'/';
   if pos('noviembre',sParamFechaTexto) > 0 then
      sCadena := sCadena +'11'+'/';
   if pos('diciembre',sParamFechaTexto) > 0 then
      sCadena := sCadena +'12'+'/';

   sCadena := sCadena + copy(sParamFechaTexto,length(sParamFechaTexto)-3,length(sParamFechaTexto));
