object FrmBuscaCorreo: TFrmBuscaCorreo
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu]
  Caption = 'Buscar Correo Elecr'#243'nico'
  ClientHeight = 262
  ClientWidth = 719
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poOwnerFormCenter
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object cxGridBusca: TcxGrid
    Left = 0
    Top = 0
    Width = 719
    Height = 227
    Align = alClient
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    ExplicitHeight = 209
    object cxViewBusca: TcxGridDBTableView
      Navigator.Buttons.CustomButtons = <>
      FilterBox.Visible = fvNever
      OnCellDblClick = cxViewBuscaCellDblClick
      DataController.DataSource = dsUsuario
      DataController.Summary.DefaultGroupSummaryItems = <>
      DataController.Summary.FooterSummaryItems = <>
      DataController.Summary.SummaryGroups = <>
      FilterRow.Visible = True
      OptionsSelection.MultiSelect = True
      OptionsView.ColumnAutoWidth = True
      OptionsView.GroupByBox = False
      OptionsView.Indicator = True
      object cxViewBuscaCorreo: TcxGridDBColumn
        Caption = 'Correo Electr'#243'nico'
        DataBinding.FieldName = 'sMail'
        Options.Editing = False
      end
      object cxViewBuscaNombre: TcxGridDBColumn
        Caption = 'Nombre'
        DataBinding.FieldName = 'sNombre'
        Options.Editing = False
      end
      object cxViewBuscaColumn2: TcxGridDBColumn
        Caption = 'Usuario'
        DataBinding.FieldName = 'sIdUsuario'
        Visible = False
        Options.Editing = False
      end
    end
    object cxGridBuscaLevel: TcxGridLevel
      GridView = cxViewBusca
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 227
    Width = 719
    Height = 35
    Align = alBottom
    TabOrder = 1
    object btnAceptar: TcxButton
      Left = 632
      Top = 1
      Width = 86
      Height = 33
      Align = alRight
      Anchors = [akTop, akRight]
      Caption = 'Aceptar'
      OptionsImage.Glyph.Data = {
        
        56B00E3226560000000600000002000000000000000000000000}
      TabOrder = 0
      OnClick = btnAceptarClick
    end
  end
  object zUsuario: TUniQuery
    Connection = connection.Uconnection
    Left = 184
    Top = 72
  end
  object dsUsuario: TDataSource
    DataSet = zUsuario
    Left = 240
    Top = 72
  end
end
