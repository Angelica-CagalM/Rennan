...
uses
  ...
  RXCtrls, RxLookup, Buttons, StrUtils, ExtCtrls,
  TeeProcs, TeEngine, Chart, Series, DbChart, DateUtils,
  ZAbstractRODataset, ZDataset, utilerias, Gauges, ExcelXP, ComObj, Excel2000,
  rtflabel, UFunctionsGHH, unitexcepciones,UnitTBotonesPermisos,
  ...
  cxCheckBox, dxCore, cxDateUtils, cxTextEdit, cxMaskEdit, cxDropDownEdit,
  cxCalendar, dxSkinOffice2016Colorful, dxSkinOffice2016Dark,
...
type
  TfrmCompara = class(TForm)
    ...
    GroupBox6: TGroupBox;
    btnMatxDia: TButton;
    frxDBDordentrabajo: TfrxDBDataset;
    Ordentrabajo: TZReadOnlyQuery;
    chkPeriodo: TcxCheckBox;
    tdIdFecha: TcxDateEdit;
    tdIdFecha1: TcxDateEdit;
   ...
   procedure formatoEncabezado();
    procedure btnMatxDiaClick(Sender: TObject);
  private
    sMenuP: String;
...
//  try
    ...
    sOrden:='';
    if connection.uconfiguracion.FieldByName('lOrdenaItem').AsString = 'Si' then
    ...
    Begin
        ...
        ActividadesxAnexo.Params.ParamByName('Convenio').Value     := sParamconvenio ;
    ...
    Begin
        ...
        begin
           showmessage('El archivo de reporte '+global_Mireporte+'_Estatus Orden.fr3 no existe, notifique al administrador del sistema');
           exit;
        end;

        if not FileExists(global_files + global_miReporte + '_Estatus OrdenDLL.fr3') then
        begin
           showmessage('El archivo de reporte '+global_Mireporte+'_Estatus OrdenDLL.fr3 no existe, notifique al administrador del sistema');
           exit;
        end;
       { if ChkPu.Checked = True Then
        ...
        ActividadesxOrden.Params.ParamByName('Convenio').Value := sParamconvenio ;
...
begin
  ...
  try
    ...
    roqOrdenes.ParamByName('status').AsString :=  'E';
    roqOrdenes.Open;
...
procedure TfrmCompara.btnMatxDiaClick(Sender: TObject);
var
  CadError, OrdenVigencia, sParamConvenio: string;
//////////////////////////////////// Seguimiento de material por dia //////////////////
  ...
  var
    ...
    var
      ...
      fs: tStream;
      Alto: Extended;
      MiFechaI, MiFechaF, MiFecha, MiFechaQuery : tDate;
      ...
      lEncuentra, lContinua : boolean;
      sColInicio, sColFinal : string;
      ProgramadoReal, ProgramadoRestante, acomulado, celda : double;
    begin
      ...
      connection.zCommand.SQL.Add('select sOficioAutorizacion, sIdConvenio from ordenesdetrabajo where sContrato =:Contrato and sNumeroOrden  like :Orden');
      connection.zCommand.ParamByName('contrato').AsString  := global_contrato;
      If MidStr(tsNumeroOrden.Text, 1 , 8) = 'CONTRATO' Then
         connection.zCommand.ParamByName('orden').AsString  := '%'
      else
         connection.zCommand.ParamByName('orden').AsString  := tsNumeroOrden.Text + '%';
      connection.zCommand.Open;

      sParamConvenio := connection.zCommand.FieldByName('sIdConvenio').AsString;
      ...
      connection.QryBusca.Active := False;
      connection.QryBusca.SQL.Clear;
      connection.QryBusca.SQL.Add('select sDescripcion, dFechaInicio as dFiProgramado, dFechaFinal as dFfProgramado from convenios where sContrato =:Contrato and sIdConvenio =:convenio and sNumeroOrden like :orden ');
      connection.QryBusca.ParamByName('Contrato').AsString := global_contrato;
      connection.QryBusca.ParamByName('Convenio').AsString := sParamConvenio;
      If MidStr(tsNumeroOrden.Text, 1 , 8) = 'CONTRATO' Then
         connection.QryBusca.ParamByName('Orden').AsString    := '%'
      else
         connection.QryBusca.ParamByName('Orden').AsString    := tsNumeroOrden.Text + '%';
      connection.QryBusca.Open;

      Hoja.Range['D'+IntTostr(Ren)+':F'+IntToStr(Ren)].Select;
      ...
      Hoja.Range['D'+IntTostr(Ren+1)+':F'+IntToStr(Ren+1)].Select;
      ...
      Excel.Selection.Value := connection.ucontrato.FieldValues['mDescripcion'];
      Excel.Selection.MergeCells := True;
      Excel.Selection.WrapText  := True;
      ...
      Excel.Selection.Value := 'CONSULTA DE MATERIALES DEL '+ DateToStr(connection.QryBusca.FieldValues['dFiProgramado']) +' AL ' + DateToSTr(tdIdFecha.Date);
      Excel.Selection.MergeCells := True;
      Excel.Selection.HorizontalAlignment := xlCenter;
      Excel.Selection.VerticalAlignment   := xlCenter;
      Excel.Selection.Font.Bold := True;
      ...
      Excel.Selection.Value := 'CANTIDAD A INSTALAR ';
      FormatoEncabezado;
      ...
      Excel.Selection.Value := 'RESTANTE';
      FormatoEncabezado;

      //Buscamos los programados de la orden..
      connection.QryBusca2.Active := False;
      connection.QryBusca2.SQL.Clear;
      connection.QryBusca2.SQL.Add('select Min(dIdFecha) as dFechaInicio, Max(dIdFecha) as dFechaFinal from avancesglobales where sContrato =:Contrato and sIdConvenio =:Convenio and sNumeroOrden like :Orden group by sContrato ');
      ...
      begin
          MiFecha  := connection.QryBusca2.FieldByName('dFechaInicio').AsDateTime;
          MiFechaI := connection.QryBusca2.FieldByName('dFechaInicio').AsDateTime;
      ...
      connection.QryBusca2.Active := False;
      connection.QryBusca2.SQL.Clear;
      connection.QryBusca2.SQL.Add('select max(dIdFecha) as dFechaFinal from reportediario where sContrato =:Contrato and sIdConvenio =:Convenio and dIdFecha <=:fecha and sNumeroOrden like :Orden ');
      connection.QryBusca2.ParamByName('contrato').AsString := global_contrato;
      connection.QryBusca2.ParamByName('Convenio').AsString := sParamConvenio;
      If MidStr(tsNumeroOrden.Text, 1 , 8) = 'CONTRATO' Then
         connection.QryBusca2.ParamByName('Orden').AsString    := '%'
      else
      ...
      connection.QryBusca.SQL.Clear;
      connection.QryBusca.SQL.Add('select i.*, i.dCantidad as dCantidadAnexo from insumos i  '+
                'where i.sContrato =:contrato and sTipoActividad = "Consumible" order by i.sIdInsumo');
      connection.QryBusca.Params.ParamByName('Contrato').DataType := ftString;
      connection.QryBusca.Params.ParamByName('Contrato').Value    := global_contrato;
      connection.QryBusca.Open;
      ...
      if connection.QryBusca.RecordCount > 0 then
      begin
        if connection.QryBusca2.FieldValues['dFechaFinal'] > MiFechaF then
           MiFechaF := connection.QryBusca2.FieldByName('dFechaFinal').AsDateTime;

        for i := 1 to (DaysBetween(MiFechaF, MiFechaI) + 1) do
        begin
            Hoja.Cells[Ren, 7 + i].Select;
           ...
           {Movimiento de la Barra..}
            ...
            begin
               ...
               // Excel.Selection.Interior.ColorIndex := 17;
                ...
                Q_Partidas.SQL.Add('select * from avancesglobales where sContrato =:Contrato and sIdConvenio =:Convenio and sNumeroOrden like :Orden');
                ...
                Q_Partidas.Params.ParamByName('Convenio').Value    := sParamConvenio;
                ...
                begin
                  ...
                  begin
                      ...
                      begin
                          if sColInicio = '' then
                             sColInicio := columnas[7 + i];
                          sColFinal := columnas[7 + i];
                          Hoja.Cells[Ren, 7 + i].Select;;
            ...
            Hoja.Range['F'+IntTostr(Ren)+':F'+IntToStr(Ren)].Select;
            Excel.Selection.Interior.ColorIndex := 45;
            ...
            Excel.Selection.NumberFormat := '##0.0000';
            Excel.Selection.Value := connection.QryBusca.FieldValues['dCantidadAnexo'];// Cantidad a instalar ****programadoReal;
            Excel.Selection.HorizontalAlignment := xlCenter;
            Excel.Selection.VerticalAlignment   := xlCenter;
            Excel.Selection.Font.Bold  := False;
            Excel.Selection.Font.size  := 14;
            ...
            Q_Partidas.Active := False;
            Q_Partidas.SQL.Clear;
            Q_Partidas.SQL.Add('select a.sNumeroOrden, b.dCantidad, b.dIdFecha from bitacorademateriales b '+
                              'inner join insumos i on (i.sContrato = b.sContrato and b.sIdMaterial = i.sIdInsumo) '+
                              'inner join bitacoradeactividades a on (a.sContrato = b.sContrato and a.dIdFecha = b.dIdFecha and a.iIdDiario = b.iIdDiario) '+
                              'where b.sContrato =:contrato and b.sIdMaterial =:material and b.dIdFecha <=:fecha order by b.sIdMaterial, b.dIdFecha');
            Q_Partidas.Params.ParamByName('Contrato').DataType := ftString;
            Q_Partidas.Params.ParamByName('Contrato').Value    := global_contrato;
            Q_Partidas.Params.ParamByName('material').DataType := ftString;
            Q_Partidas.Params.ParamByName('material').Value    := connection.QryBusca.FieldValues['sIdInsumo'];
            Q_Partidas.Params.ParamByName('fecha').DataType    := ftDate;
            Q_Partidas.Params.ParamByName('fecha').Value       := tdIdFecha.Date;
            Q_Partidas.Open;

            celda := 0;
            if Q_Partidas.FieldByName('dIdFecha').AsDateTime < MiFechaI then
            begin
               MiFecha := Q_Partidas.FieldByName('dIdFecha').AsDateTime;
               celda   := (MiFechaI - Q_Partidas.FieldByName('dIdFecha').AsDateTime);
            end
            else
               MiFecha := MiFechaI;
            if Q_Partidas.RecordCount > 0 then
            begin
              dVolumen := 0;
              i := 1;
              while i <= (DaysBetween(MiFechaF, MiFechaI) + (celda + 1)) do
              begin
                  MiFechaQuery := 0;
                  if MiFecha = Q_Partidas.FieldByName('dIdFecha').AsDateTime then
                  begin
                      ...
                      If MidStr(tsNumeroOrden.Text, 1 , 8) = 'CONTRATO' Then
                         ...
                         if Q_Partidas.FieldValues['sNumeroOrden'] = tsNumeroOrden.Text then
                            lContinua := True;
                      ...
                      begin
                          if Q_Partidas.FieldByName('dIdFecha').AsDateTime >= MiFechaI then
                          begin
                              Hoja.Cells[Ren + 1, 7 + (i - celda)].Select;
                              Excel.Selection.NumberFormat := '##0.0000';
            ...
            Excel.Selection.NumberFormat := '##0.0000';
            if connection.QryBusca.FieldValues['dCantidadAnexo'] <= dVolumen then
               Excel.Selection.Value := 0
            else
            begin
               Excel.Selection.Value := connection.QryBusca.FieldValues['dCantidadAnexo'] - dVolumen;
            end;
            Excel.Selection.HorizontalAlignment := xlCenter;
            Excel.Selection.VerticalAlignment   := xlCenter;
            Excel.Selection.Font.Bold  := False;
      ...
      Hoja.Range['A'+IntTostr(1)+':F'+IntToStr(1)].Select;
  ...
  begin
    ...
    try
      ...
      Hoja.Select;
      try
        Hoja.Name := 'MATERIALES PROGRAMA ' + tsNumeroOrden.Text;
      except
      ...
      begin
        Resultado := False;
        CadError := 'Se ha producido el siguiente error al Generar los materiales:' + #10 + #10 + e.Message;
...
begin
 try
  ...
  If MidStr(tsNumeroOrden.Text, 1 , 8) = 'CONTRATO' then
     ...
     sParamconvenio := roqOrdenes.FieldByName('sIdConvenio').AsString;
 except
  on e : exception do begin
...
begin
  ...
  try
    ...
    Begin
        ...
        Reporte.Params.ParamByName('Convenio').Value := sParamconvenio ;
        dsInforme.FieldAliases.Clear ;
    ...
    Begin
     begin
        ...
        Reporte.Params.ParamByName('Mes').Value := 0;
        Reporte.Params.ParamByName('Anno').DataType := ftInteger ;
        Reporte.Params.ParamByName('Anno').Value := 0 ;
        dsInforme.FieldAliases.Clear ;
        dsInforme.DataSet := Reporte ;
        Reporte.Open ;
     end;
...
begin
  ...
  try
    ...
    connection.qryBusca2.ParamByName('convenio').Value := sParamconvenio;
    Connection.qryBusca2.Open ;
    ...
    begin
      ...
      Begin
          ...
          begin
              ...
              qryBuscaP.Params.ParamByName('Convenio').Value := sParamconvenio ;
...
begin
  ...
  begin
    if CompareText(VarName, 'DESC_CONTRATO') = 0 then
      Value := Connection.ucontrato.FieldByName('mDescripcion').AsString;
...
begin
          ...
          connection.QryBusca.ParamByName('convenio').Value := sParamconvenio;
          connection.QryBusca.Open;
