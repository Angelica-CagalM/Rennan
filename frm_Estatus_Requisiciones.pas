unit frm_Estatus_Requisiciones;
...
type
  TfrmEstatus_Requisiciones = class(TForm)
    PanelCentral: TPanel;
    ...
    cxView_RequsicionColumn15: TcxGridDBColumn;
    cxView_RequsicionColumn16: TcxGridDBColumn;
    cxView_RequsicionColumn17: TcxGridDBColumn;
    cxView_RequsicionColumn18: TcxGridDBColumn;
    cxView_RequsicionColumn19: TcxGridDBColumn;
...
var
  lExpandido : boolean;
  frmEstatus_Requisiciones: TfrmEstatus_Requisiciones;

implementation
  uses frm_connection, unitGenerales,global;
{$R *.dfm}

procedure TfrmEstatus_Requisiciones.btnExportaClick(Sender: TObject);
var
  rutaArchivo: string;
  Excel, Sheet: Variant;
  dlgSave : TSaveDialog;
...
procedure TfrmEstatus_Requisiciones.btnPrinterClick(Sender: TObject);
 var
 listaProv : String;
begin
...
procedure TfrmEstatus_Requisiciones.btnRefreshClick(Sender: TObject);
begin
anexo_prequisicion.Refresh;
end;

procedure TfrmEstatus_Requisiciones.cxLimpiaFiltroClick(Sender: TObject);
begin
...
procedure TfrmEstatus_Requisiciones.cxButton1Click(Sender: TObject);
begin
   if lExpandido then
   begin
...
procedure TfrmEstatus_Requisiciones.ValidacionUsuarios;
begin
...
procedure TfrmEstatus_Requisiciones.cxFiltroRequeridoClick(Sender: TObject);
begin
    if cxOpcionesFiltro.ItemIndex = 0 then
    begin
        anexo_prequisicion.Filtered := False;
...
procedure TfrmEstatus_Requisiciones.cxView_RequsicionCellClick(
...
procedure TfrmEstatus_Requisiciones.cxView_RequsicionCellDblClick(
  Sender: TcxCustomGridTableView; ACellViewInfo: TcxGridTableDataCellViewInfo;
  AButton: TMouseButton; AShift: TShiftState; var AHandled: Boolean);
begin
...
procedure TfrmEstatus_Requisiciones.cxView_RequsicionStylesGetContentStyle(
  Sender: TcxCustomGridTableView; ARecord: TcxCustomGridRecord;
  AItem: TcxCustomGridTableItem; var AStyle: TcxStyle);
  var
...
procedure TfrmEstatus_Requisiciones.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  guardar_leer_grid(cxView_Requsicion,frmEstatus_Requisiciones,1);
  action:=cafree;
end;

procedure TfrmEstatus_Requisiciones.FormShow(Sender: TObject);
var
 temE:TUniQuery;
begin
  ...
  FiltrarDataSet(anexo_prequisicion,'contrato,Folio,Tipo',[global_contrato, '-1','-1'])
  else
...
//  else
  ...
  guardar_leer_grid(cxView_Requsicion,frmEstatus_Requisiciones,0);

end;

procedure TfrmEstatus_Requisiciones.frxEntradaGetValue(const VarName: string;
  var Value: Variant);
begin
