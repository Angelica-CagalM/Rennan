...
var
  frm_Empresa_SSI: Tfrm_Empresa_SSI;
  titulo : String;
  btnPermisos: String;
...
procedure Tfrm_Empresa_SSI.GlobalKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrm_Empresa_SSI.EnterControl(Sender: TObject);
begin
...
procedure Tfrm_Empresa_SSI.SalidaControl(Sender: TObject);
begin
    if (sender is tcxDBTextEdit) then
...
procedure Tfrm_Empresa_SSI.btnAddClick(Sender: TObject);
begin
  try
    try
      zEmpresa.Append;
...
procedure Tfrm_Empresa_SSI.btnAgregarClick(Sender: TObject);
begin
  PanelData.Visible:=true;
  cuentas.Append;
end;

procedure Tfrm_Empresa_SSI.btnCancelClick(Sender: TObject);
begin
  try
    zEmpresa.Cancel;
  finally
    ...
    pnl1.Visible := False;
    PermisosBotones(frm_Empresa_SSI,btnPermisos);
  end;
end;

procedure Tfrm_Empresa_SSI.btnCancelDetClick(Sender: TObject);
begin
  Cuentas.Cancel;
  PanelData.Visible:=False;
end;

procedure Tfrm_Empresa_SSI.btnDeleteClick(Sender: TObject);
begin
  ...
  if MessageDlg('¿Desea eliminar el registro seleccionado?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
   begin
    try
      ...
      Connection.zUCommand.SQL.text:='UPDATE master_empresa SET Activo=:activo WHERE (IdEmpresa=:Id)';
      Connection.zUCommand.ParamByName('activo').AsString:='No';
      Connection.zUCommand.ParamByName('Id').AsInteger:=zEmpresa.FieldByName('IdEmpresa').AsInteger;
...
procedure Tfrm_Empresa_SSI.btnEditarClick(Sender: TObject);
begin
 PanelData.Visible:=true;
 Cuentas.Edit;
end;

procedure Tfrm_Empresa_SSI.btnEditClick(Sender: TObject);
begin
  try
    try
      zEmpresa.Edit;
...
procedure Tfrm_Empresa_SSI.btnEliminarClick(Sender: TObject);
begin
...
procedure Tfrm_Empresa_SSI.btnPostClick(Sender: TObject);
begin
  try
    try
...
procedure Tfrm_Empresa_SSI.btnPostDetClick(Sender: TObject);
begin
  Cuentas.Post;
  btnCancelDetClick(Sender);
end;

procedure Tfrm_Empresa_SSI.btnRefreshClick(Sender: TObject);
begin
   zEmpresa.Refresh;
end;

procedure Tfrm_Empresa_SSI.cbbCiudadPropertiesEditValueChanged(Sender: TObject);
begin
...
procedure Tfrm_Empresa_SSI.cuentasAfterInsert(DataSet: TDataSet);
begin
    cuentas.FieldByName('IdCuenta').AsInteger := 0;
  cuentas.FieldByName('IdEmpresa').AsInteger := zEmpresa.FieldByName('IdEmpresa').AsInteger;
end;

procedure Tfrm_Empresa_SSI.cxDBButtonEdit1PropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  try
...
procedure Tfrm_Empresa_SSI.cxMapaDblClick(Sender: TObject);
var
...
procedure Tfrm_Empresa_SSI.EdithBtnKeyPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
   try
    if tDialogo.Execute then
...
procedure Tfrm_Empresa_SSI.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  try
...
procedure Tfrm_Empresa_SSI.FormShow(Sender: TObject);
var
    paramNames : string;
...
procedure Tfrm_Empresa_SSI.ImgCedulaDblClick(Sender: TObject);
begin
  tcxImage(ImgCedula).LoadFromFile;
end;

procedure Tfrm_Empresa_SSI.imgEmpDblClick(Sender: TObject);
begin
