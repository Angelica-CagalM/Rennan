...
//   zFolios: TUniQuery;
 ...
 // Procedure procAutorizaDatosValidaAutoriza(sParamContrato, sParamTabla, sTipo :string; iParamFolio :integer; tOrigen: TComponent );
  ...
  Function fReemplazaFiltroSQL(sParamSQL : WideString ; sParamFiltro, sParamVar1, sParamVar2, sParamVar3, sParamVar4, sParamVar5 :string):WideString;
...
begin
    ...
    begin
        ...
        begin
           zqValida.Active:= False;
           AsignarSQL(zqValida,'Update_autoriza_presupeusto',pUpDate);
        end;
...
begin
    ...
    Try
      ...
      if trim(sParamContrato) <> '' then
      begin
         QryMaximo.SQL.Add('select '+sParamCampo+' from '+sParamTabla+' where sContrato =:Contrato and '+sParamCampo+' =:Id ' );
         QryMaximo.ParamByName('contrato').AsString := sParamContrato;
         QryMaximo.ParamByName('Id').AsString       := sParamValor;
      end
      else
      begin
         QryMaximo.SQL.Add('select '+sParamCampo+' from '+sParamTabla+' where '+sParamCampo+' =:Id ');
         QryMaximo.ParamByName('Id').AsString       := sParamValor;
      end;
      QryMaximo.Open;
...
var
  ...
  sParamFilter, sParamVar1, sParamVar2, sParamVar3, sParamVar4, sParamVar5, sParamSentencia : string;
begin
  try
    Result := False;
    ...
    sParamSentencia := fReemplazaFiltroSQL(Dataset.SQL.Text,sParamFilter, sParamVar1, sPAramVar2, sPAramVar3, sParamVar4, sParamVar5);
...
begin
  ...
  zFolios.SQL.Add('Select '+
      ...
      'f.Campo,  '+
      'f.IniciarEn, '+
      '(select prefijo from master_cnf_folios where tabla =t.TABLE_NAME and IdEmpresa =:Empresa and IdEmbarcacion =:Barco and IdDEpartamento =:Depto ) as Prefijo, '+
      '(select Aplicar from master_cnf_folios where tabla =t.TABLE_NAME and IdEmpresa =:Empresa and IdEmbarcacion =:Barco and IdDEpartamento =:Depto ) as Aplicar,  '+
      '(select complemento from master_cnf_folios where tabla =t.TABLE_NAME and IdEmpresa =:Empresa and IdEmbarcacion =:Barco and IdDEpartamento =:Depto) as complemento, '+
      '(select numDig from master_cnf_folios where tabla =t.TABLE_NAME and IdEmpresa =:Empresa and IdEmbarcacion =:Barco and IdDEpartamento =:Depto) as NumDig,       '+
      '(select Aux from master_cnf_folios where tabla =t.TABLE_NAME and IdEmpresa =:Empresa and IdEmbarcacion =:Barco and IdDEpartamento =:Depto ) as Aux,                '+
      '(select IniciarEn from master_cnf_folios where tabla =t.TABLE_NAME and IdEmpresa =:Empresa and IdEmbarcacion =:Barco and IdDEpartamento =:Depto ) as IniciarEn,  '+
      't.TABLE_COMMENT, '+
      't.TABLE_NAME,    '+
      'c.COLUMN_NAME,   '+
      'c.IS_NULLABLE,   '+
