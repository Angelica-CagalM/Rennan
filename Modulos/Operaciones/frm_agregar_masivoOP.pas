...
uses
  ...
  dxBarBuiltInMenu, cxPC, dxLayoutControlAdapters, unitMetodos ;

type
  TfrmAgregarMasivoOP = class(TForm)
    mnuPop: TPopupMenu;
    Insertar1: TMenuItem;
    Editar1: TMenuItem;
    N1: TMenuItem;
    ...
    zInsumoCC: TUniQuery;
    dsInsumoCC: TDataSource;
    ProcExistenCC: TUniStoredProc;
    ...
    View_Grid_MaterialesCentroCosto: TcxGridDBColumn;
...
var
  frmAgregarMasivoOP : TfrmAgregarMasivoOP;
  sOldId           : string;
  sOpcion          : string;
  Parametro        : string;
  Id               : string;
  ...
  implementation
 uses
    frm_catalogos_Operaciones,Frm_CostoPresupuesto,Frm_RequisicionMaterial,frm_SalidaAlmacen,frm_AvisoEmbarque,
  frm_EntradaAlmacen,frm_Existencia, Frm_SolicitudMaterial, frm_SolicitudDeMateriales,Func_Genericas;

{$R *.dfm}


procedure TfrmAgregarMasivoOP.addAsignacion;
begin
...
procedure TfrmAgregarMasivoOP.addRequisicon;
Var
  SavePlace : TBookmark;
...
//        try
          ...
          begin
              ...
              Connection.QryUBusca.Params.ParamByName('IdTipoRecurso').AsInteger     :=  FrmRequisicionMaterial.anexo_requisicion.FieldByName('IdTipoRecurso').AsInteger;
          ...
          begin
             ...
             begin
                ...
                Connection.zUCommand.SQL.Add('Select Max(iItem) as iItem From anexo_prequisicion Where sContrato = :Contrato And iFolioRequisicion = :Folio and IdInsumo = :Insumo ');
                connection.zUCommand.Params.ParamByName('Contrato').AsString := Global_Contrato;
                connection.zUCommand.Params.ParamByName('Folio').AsInteger   := FrmRequisicionMaterial.anexo_requisicion.FieldByName('iFolioRequisicion').AsInteger;
...
Procedure TfrmAgregarMasivoOP.SelectEntradas;
begin
    //Entrada de Orden de Compra(ENOC)
    if frmEntradaAlmacen.almacen_entrada.FieldByName('CodigoEntrada').AsString = 'ENOC' then
    begin
        ...
        [global_contrato,frmEntradaAlmacen.almacen_entrada.FieldByName('iFolioMovimiento').AsInteger,frmEntradaAlmacen.almacen_entrada.FieldByName('IdAlmacen').AsInteger]);
        materiales.Open;
    ...
    if (frmEntradaAlmacen.almacen_entrada.FieldByName('CodigoEntrada').AsString = 'TRAE') or (frmEntradaAlmacen.almacen_entrada.FieldByName('CodigoEntrada').AsString = 'RECP') or (frmEntradaAlmacen.almacen_entrada.FieldByName('CodigoEntrada').AsString = 'EDEV')then
    begin
        materiales.Active := False;
        AsignarSQL(materiales,'traspaso_entrada',pUpdate);
        FiltrarDataSet(materiales,'Contrato,Almacen,Folio',
        [global_contrato,frmEntradaAlmacen.almacen_entrada.FieldByName('IdAlmacen').AsInteger, frmEntradaAlmacen.almacen_entrada.FieldByName('iFolioMovimiento').AsString]);
        materiales.Open;
    end
    else
    //Entrada por Aviso de Embarque(ENAE)
    if ((connection.uContrato.FieldByName('FK_Titulo').AsString='SUBSEA 7') and (frmEntradaAlmacen.almacen_entrada.FieldByName('CodigoEntrada').AsString = 'ENAE')) then
    begin
        materiales.Active := False;
    ...
    if frmEntradaAlmacen.almacen_entrada.FieldByName('CodigoEntrada').AsString = 'EINI' then
    begin
         materiales.Active := False;
        AsignarSQL(materiales,'insumo_inicial_entrada',pUpdate);
        FiltrarDataSet(materiales,'Contrato,Usuario,Almacen',
        [global_contrato,global_usuario,frmEntradaAlmacen.almacen_entrada.FieldByName('IdAlmacen').AsInteger]);
        materiales.Open;
    end
    else
    begin
        ...
        FiltrarDataSet(materiales,'Contrato,Usuario,sNumeroOrdenCC',
        [global_contrato,global_usuario, frmEntradaAlmacen.almacen_entrada.FieldByName('sNumeroOrdenCC').AsString]);
        materiales.Open;
    end;
    if ((connection.uContrato.FieldByName('FK_Titulo').AsString='SUBSEA 7') AND (frmEntradaAlmacen.almacen_entrada.FieldByName('CodigoEntrada').AsString = 'ENAE')) then
    begin
...
procedure TfrmAgregarMasivoOP.addEntrada;
var
...
begin
   ...
   if (frmEntradaAlmacen.almacen_entrada.FieldByName('Proviene').AsString <> 'N/A') then
   begin
      ...
      begin
         ...
         begin
           if connection.uContrato.FieldByName('FK_Titulo').AsString='TYPHOON' then
            begin
             ...
             [frmEntradaAlmacen.almacen_entrada.FieldByName('sContrato').AsString,frmEntradaAlmacen.almacen_entrada.FieldByName('iFolioMovimiento').AsInteger,
         ...
         if frmEntradaAlmacen.almacen_entrada.FieldByName('CodigoEntrada').AsString = 'ENAE' then
         begin
       ...
       (frmEntradaAlmacen.almacen_entrada.FieldByName('CodigoEntrada').AsString = 'ENOC')then
       BEGIN

           Entradas.Active := False;
           asignarSQL(Entradas,'cant_bitacoraentrada_pedido',pUpdate);
           FiltrarDataSet(Entradas,'Contrato,Folio,Insumo,FolioCompra,item',
           [frmEntradaAlmacen.almacen_entrada.FieldByName('sContrato').AsString,frmEntradaAlmacen.almacen_entrada.FieldByName('iFolioEntrada').AsInteger,
      ...
      if frmEntradaAlmacen.almacen_entrada.FieldByName('CodigoEntrada').AsString <> 'EINI' then
          if (sumCantidades > CantidadPedido) and ((frmEntradaAlmacen.almacen_entrada.FieldByName('CodigoEntrada').AsString <> 'TRAE') and (frmEntradaAlmacen.almacen_entrada.FieldByName('CodigoEntrada').AsString <> 'EMAT') and (frmEntradaAlmacen.almacen_entrada.FieldByName('CodigoEntrada').AsString <> 'EDEV'))then
          begin
            messageDLG('No se puede Recibir mas Material que lo Comprado, Favor de Verificar.', mtInformation, [mbOk], 0);
            exit;
          end;

      if edtCantidad.Value <= 0 then
              begin
                      messageDLG('No se puede dar una entrada con una cantidad menor o igual a 0 !', mtInformation, [mbOk],0);
                      exit;
              end              ;
   end;
    ...
    begin
       ...
       if (frmEntradaAlmacen.almacen_entrada.FieldByName('CodigoEntrada').AsString = 'EINI') or (frmEntradaAlmacen.almacen_entrada.FieldByName('Proviene').AsString = 'N/A') or (frmEntradaAlmacen.almacen_entrada.FieldByName('Proviene').AsString = ' S / R ') then
       begin
          FiltrarDataSet(Entradas,'Contrato,Insumo,Almacen,Folio,FolioReq',
          [global_contrato,materiales.FieldByName('IdInsumo').AsInteger,frmEntradaAlmacen.almacen_entrada.FieldByName('IdAlmacen').AsInteger,
          frmEntradaAlmacen.almacen_entrada.FieldByName('iFolioEntrada').AsInteger,0]);
       end
       else
       begin
          if Connection.uContrato.FieldByName('sMascara').AsString = 'SUBSEA 7' then
            ...
            [global_contrato,materiales.FieldByName('IdInsumo').AsInteger,frmEntradaAlmacen.almacen_entrada.FieldByName('IdAlmacen').AsInteger,
            frmEntradaAlmacen.almacen_entrada.FieldByName('iFolioEntrada').AsInteger])
          else
            FiltrarDataSet(Entradas,'Contrato,Insumo,Almacen,Folio,FolioReq',
            [global_contrato,materiales.FieldByName('IdInsumo').AsInteger,frmEntradaAlmacen.almacen_entrada.FieldByName('IdAlmacen').AsInteger,
            frmEntradaAlmacen.almacen_entrada.FieldByName('iFolioEntrada').AsInteger,materiales.FieldByName('iFolioRequisicion').AsInteger]);
       end;
       Entradas.Open;
      ...
      try
        ...
        sumCant := CantidadAnterior + edtCantidad.Value ;
          ...
          begin
            ...
            [frmEntradaAlmacen.almacen_entrada.FieldByName('sContrato').AsVariant,
            0,frmEntradaAlmacen.almacen_entrada.FieldByName('iFolioEntrada').AsInteger,
            materiales.FieldByName('IdInsumo').AsInteger,
            FechaSQL(frmEntradaAlmacen.almacen_entrada.FieldValues['dFechaEntrega']),edtCantidad.Value,
            CantidadAnterior,sumCant,
            frmEntradaAlmacen.almacen_entrada.FieldByName('IdAlmacen').AsInteger,'',0])
          end
          else begin
            ...
            begin
              if ((Connection.uContrato.FieldByName('sMascara').AsString = 'COMPAÑIA CMMI') and (frmEntradaAlmacen.almacen_entrada.FieldByName('CodigoEntrada').AsString = 'ENOC')) then
                FiltrarDataSet(zActualiza,'Contrato,Item,Folio,Insumo,FechaE,Cantidad,CantidadAnterior,CantidadTotal,IdAlmacen,Comentario, FolioReq',
                [frmEntradaAlmacen.almacen_entrada.FieldByName('sContrato').AsString,0,frmEntradaAlmacen.almacen_entrada.FieldValues['iFolioEntrada'],
                materiales.FieldByName('IdInsumo').AsInteger,FechaSQL(frmEntradaAlmacen.almacen_entrada.FieldValues['dFechaEntrega']),edtCantidad.Value,
                CantidadAnterior,sumCant,frmEntradaAlmacen.almacen_entrada.FieldByName('IdAlmacen').AsInteger,
                ...
                FiltrarDataSet(zActualiza,'Contrato,Item,Folio,Insumo,FechaE,Cantidad,CantidadAnterior,CantidadTotal,IdAlmacen,Comentario,FolioReq',
                [frmEntradaAlmacen.almacen_entrada.FieldByName('sContrato').AsString,materiales.FieldByName('iItem').AsInteger,frmEntradaAlmacen.almacen_entrada.FieldValues['iFolioEntrada'],
                materiales.FieldByName('IdInsumo').AsInteger,FechaSQL(frmEntradaAlmacen.almacen_entrada.FieldValues['dFechaEntrega']),edtCantidad.Value,
                CantidadAnterior,sumCant,frmEntradaAlmacen.almacen_entrada.FieldByName('IdAlmacen').AsInteger,
                '',materiales.FieldByName('iFolioRequisicion').AsInteger]);
          end;
      ...
      begin
        ...
        begin
           ...
           [sumCant,frmEntradaAlmacen.almacen_entrada.FieldByName('sContrato').AsString,
           frmEntradaAlmacen.almacen_entrada.FieldByName('iFolioEntrada').AsInteger,
           materiales.FieldByName('IdInsumo').AsInteger]);
           zActualiza.ExecSQL;
           lContinua := True;
            ...
            begin
              ...
              begin
                ...
                begin
                    ...
                    if ((Connection.uContrato.FieldByName('sMascara').AsString = 'SUBSEA 7') or (Connection.uContrato.FieldByName('sMascara').AsString = 'COMPAÑIA CMMI')) then
                      FiltrarDataSet(Entradas,'Contrato,Folio,Insumo,FolioReq',
                      [frmEntradaAlmacen.almacen_entrada.FieldByName('sContrato').AsString, frmEntradaAlmacen.almacen_entrada.FieldValues['iFolioMovimiento'],
                      materiales.FieldByName('IdInsumo').AsInteger,materiales.FieldByName('iFolioRequisicion').AsInteger])
                   else
                ...
                begin
                    ...
                    [frmEntradaAlmacen.almacen_entrada.FieldValues['iFolioMovimiento'],materiales.FieldByName('IdInsumo').AsInteger]);
                    Entradas.ExecSQL;
                end;

                //Traspaso Entrada (TRAE) o Entrada por recepcion de material (RECP)
                if (frmEntradaAlmacen.almacen_entrada.FieldByName('CodigoEntrada').AsString = 'TRAE') or (frmEntradaAlmacen.almacen_entrada.FieldByName('CodigoEntrada').AsString = 'RECP') then
                begin
  ...
  if (frmEntradaAlmacen.almacen_entrada.FieldByName('CodigoEntrada').AsString <> 'EMAT') AND (frmEntradaAlmacen.almacen_entrada.FieldByName('CodigoEntrada').AsString <> 'TRAE') and (frmEntradaAlmacen.almacen_entrada.FieldByName('CodigoEntrada').AsString <> 'EINI') then //and (frmEntradaAlmacen.almacen_entrada.FieldByName('IdTipo').AsInteger <> 3)then
  begin
    materiales.Refresh;
  ...
  frmEntradaAlmacen.pEntradas.Refresh;
  frmEntradaAlmacen.almacen_entrada.RefreshRecord;
  frmEntradaAlmacen.PgControl.Enabled:=True;
  frmEntradaAlmacen.Grid_Entradas.Enabled := True;

end;

procedure TfrmAgregarMasivoOP.addEntradaMasivo;
var
  CantidadPedido, CantidadEntrada, total, sumCant,
  CantidadAnterior, CantidadPosterior, sumCantidades: double;
  PosInsumo :TBookmark;
...
begin
    ...
    begin
       ...
       if (Entradas.RecordCount = 0) or ((connection.uContrato.FieldByName('FK_Titulo').AsString = 'SUBSEA 7') and
          (Entradas.RecordCount > 0)  and ((frmEntradaAlmacen.almacen_entrada.FieldByName('Tipo').AsString = 'AVISO DE EMBARQUE') OR
          (frmEntradaAlmacen.almacen_entrada.FieldByName('Tipo').AsString = 'AVISO DE DESEMBARQUE') ))then
       begin
          sumCant := CantidadAnterior + edtCantidad.Value ;
          zActualiza.Active := False;
          ...
          begin
            ...
            0,frmEntradaAlmacen.almacen_entrada.FieldByName('iFolioEntrada').AsInteger,
            materiales.FieldByName('IdInsumo').AsInteger,
            FechaSQL(frmEntradaAlmacen.almacen_entrada.FieldValues['dFechaEntrega']),materiales.FieldByName('dCantidadPendiente').AsFloat,
          ...
          begin
            if connection.uContrato.FieldByName('FK_Titulo').AsString='SUBSEA 7' then
              ...
              [frmEntradaAlmacen.almacen_entrada.FieldByName('sContrato').AsString,'0',frmEntradaAlmacen.almacen_entrada.FieldValues['iFolioEntrada'],
              materiales.FieldByName('IdInsumo').AsInteger,FechaSQL(frmEntradaAlmacen.almacen_entrada.FieldValues['dFechaEntrega']),materiales.FieldByName('dCantidadPendiente').AsFloat,
              materiales.FieldByName('dCantidad').AsFloat,frmEntradaAlmacen.almacen_entrada.FieldByName('IdAlmacen').AsInteger,
              ''])
            else
              FiltrarDataSet(zActualiza,'Contrato,Item,Folio,Insumo,FechaE,Cantidad,CantidadAnterior,CantidadTotal,IdAlmacen,Comentario,FolioReq',
              [frmEntradaAlmacen.almacen_entrada.FieldByName('sContrato').AsString,'0',frmEntradaAlmacen.almacen_entrada.FieldValues['iFolioEntrada'],
              materiales.FieldByName('IdInsumo').AsInteger,FechaSQL(frmEntradaAlmacen.almacen_entrada.FieldValues['dFechaEntrega']),materiales.FieldByName('dCantidadPendiente').AsFloat,
              materiales.FieldByName('dCantidadRecibida').AsFloat,materiales.FieldByName('dCantidad').AsFloat,frmEntradaAlmacen.almacen_entrada.FieldByName('IdAlmacen').AsInteger,
              '',materiales.FieldByName('iFolioRequisicion').AsInteger]);
          end;
...
procedure TfrmAgregarMasivoOP.addInsumosSolicitud;
...
//        try
          ...
          Connection.QryUBusca.Active := False;
          Connection.QryUBusca.SQL.Clear;
          Connection.QryUBusca.SQL.Add('select IdInsumo from anexo_psolicitud where sContrato =:Contrato and sNumeroOrdenCC=:CC and iFolioSolicitud =:Folio and IdInsumo =:Insumo '+
          'and (:Tipo = -1 or (:Tipo <> -1 and  IdTipoRecurso = :Tipo))');
          Connection.QryUBusca.Params.ParamByName('Contrato').AsString := Global_Contrato;
          ...
          Connection.QryUBusca.Params.ParamByName('Insumo').AsString   := materiales.FieldByName('IdInsumo').AsString;
          Connection.QryUBusca.Params.ParamByName('CC').AsString   := View_Grid_Materiales.DataController.DataSet.FieldByName('sNumeroOrdenCC').AsString;
...
//          end;
          ...
          begin
              ...
              Connection.QryUBusca.Active := False;
              Connection.QryUBusca.SQL.Clear;
              Connection.QryUBusca.SQL.Add('INSERT INTO anexo_psolicitud(sContrato,IdPSolicitud,iFolioSolicitud,IdInsumo,iItem,mDescripcion,IdMedida,dFechaRequerimiento,dCantidad,sObservacion,sEstado,IdTipoRecurso, sNumeroOrdenCC) VALUES '+
                                                                        '(:Contrato, 0, :iFolioSolicitud, :IdInsumo, 0, :Descripcion, :IdMedida, :dFechaReq,:Cantidad,:Observa, :sStatus, :IdTipoRecurso, :CC)');
              ...
              Connection.QryUBusca.ParamByName('CC').AsString := materiales.FieldByName('sNumeroOrdenCC').AsString;
              ...
              begin
                ...
                if cant <= 0 then begin
                   messageDLG('Ya no hay existencia de materiales para reservar!', mtInformation, [mbOK], 0);
...
//                connection.qryUBusca.Params.ParamByName('dCantidadSolicitado').AsFloat := 0;
              ...
              if edtCantidad.Value <= 0 then
              begin
                      messageDLG('No se puede dar una solicitud con una cantidad menor que 0 !', mtInformation, [mbOk],0);
                      Guardar := False;
              end              ;
          ...
          begin
            ...
            begin
              Connection.zUCommand.Active := False;
              Connection.zUCommand.SQL.Clear;
              Connection.zUCommand.SQL.Add('Select Max(iItem) as iItem From anexo_psolicitud Where sContrato = :Contrato And iFolioSolicitud = :Folio and IdInsumo = :Insumo and sNumeroOrdenCC=:CC group by iItem');
              connection.zUCommand.Params.ParamByName('Contrato').AsString := Global_Contrato;
              ...
              Connection.zUCommand.Params.ParamByName('Insumo').AsString   := materiales.FieldByName('IdInsumo').AsString;
              Connection.zUCommand.ParamByName('CC').AsString := materiales.FieldByName('sNumeroOrdenCC').AsString;
              Connection.zUCommand.Open;
              ...
              begin
                ...
                Connection.QryUBusca.SQL.Clear;
                Connection.QryUBusca.SQL.Add('INSERT INTO anexo_psolicitud(sContrato,IdPSolicitud,iFolioSolicitud,IdInsumo,iItem,mDescripcion,IdMedida,dFechaRequerimiento,dCantidad,sObservacion,sEstado,IdTipoRecurso, sNumeroOrdenCC) VALUES '+
                                                                          '(:Contrato, 0, :iFolioSolicitud, :IdInsumo, :Item, :Descripcion, :IdMedida, :dFechaReq,:Cantidad,:Observa, :sStatus, :IdTipoRecurso, :CC )');
                Connection.QryUBusca.ParamByName('Contrato').AsString:=FrmSolicitudMaterial.anexo_solicitud.FieldByName('sContrato').AsString;
                Connection.QryUBusca.ParamByName('iFolioSolicitud').AsInteger:=FrmSolicitudMaterial.anexo_solicitud.FieldByName('iFolioSolicitud').AsInteger;
                Connection.QryUBusca.ParamByName('IdInsumo').AsInteger:=materiales.FieldByName('IdInsumo').AsInteger;
                Connection.QryUBusca.ParamByName('Descripcion').AsString:=materiales.FieldByName('Insumo').AsString;
...
//                Connection.QryUBusca.ParamByName('Cantidad').AsFloat:=edtCantidad.Value;
            ...
            begin
              ...
              Connection.qryUBusca.SQL.Add('Select dCantidad From anexo_psolicitud Where sContrato = :Contrato And ' +
                'iFolioSolicitud = :Folio and IdInsumo = :Insumo and sNumeroOrdenCC = :CC ');
              connection.qryUBusca.Params.ParamByName('Contrato').AsString := Global_Contrato;
              ...
              Connection.zUCommand.SQL.Add('UPDATE anexo_psolicitud SET dCantidad =:Cantidad ' +
                'WHERE sContrato =:Contrato And iFolioSolicitud =:Folio and IdInsumo =:Insumo And iItem = 0 and sNumeroOrdenCC= :CC');
              Connection.zUCommand.Params.ParamByName('Contrato').AsString := Global_Contrato;
              Connection.zUCommand.Params.ParamByName('Folio').AsString    := FrmSolicitudMaterial.anexo_solicitud.FieldByName('iFolioSolicitud').AsString;
              Connection.zUCommand.Params.ParamByName('Insumo').AsString   := materiales.FieldByName('IdInsumo').AsString;
              Connection.zUCommand.Params.ParamByName('CC').AsString := materiales.FieldByName('sNumeroOrdenCC').AsString;
...
procedure TfrmAgregarMasivoOP.cargarExistenciCC;
var sqlCC: TUniquery;
begin
    ProcExistenCC.Active := False ;
    ProcExistenCC.Params.parambyname('Contrato').AsString := 'CMMI';
    ProcExistenCC.Execute;
    AsignarSQL(zInsumoCC,'procedimiento_existenciaCC',pUpdate);
    FiltrarDataset(zInsumoCC,'Contrato',[global_contrato]);
    zInsumoCC.Open;
end;

procedure TfrmAgregarMasivoOP.addSalidaEmbarque;
var
...
//    connection.qryUBusca.ParamByName('Almacen').AsInteger  := zDetalleMaterial.FieldByName('IdAlmacen').AsInteger;
    ...
    if frmAvisoEmbarque.zSalidaInsumo.RecordCount >= 0 then begin
...
//       end;
       if connection.qryUBusca.FieldValues['dExistencia'] = 0 then begin
        ...
        frmAvisoEmbarque.Cancelar.Click;
        exit;
       end;
    end;
...
//    begin
        ...
        if (materiales.FieldByName('CantSalida').AsFloat <> 0) and ((materiales.FieldByName('CantSalida').AsFloat <> null))then
         connection.zUCommand.Params.ParamByName('cantidad').AsFloat  := materiales.FieldByName('CantSalida').AsFloat
        else if (connection.qryUBusca.FieldValues['dExistencia'] > materiales.FieldByName('dCantidad').AsFloat) or
...
procedure TfrmAgregarMasivoOP.addSalidaSolicitudMateriales;
var
  dActExistencia:Extended;
begin
  dActExistencia:=0;
...
procedure TfrmAgregarMasivoOP.addSalida;
var
  dActExistencia:Extended;
  zDatos :TUniQuery;
begin
  ...
  begin
    dActExistencia:=0;
      ...
      Begin
        ...
        if connection.uContrato.FieldByName('FK_Titulo').AsString='TYPHOON' then
        begin
          connection.qryUBusca.SQL.Add('select Existencias(i.IdInsumo, :Almacen, :Contrato,:Item) AS dExistencia'+
          ' from alm_insumos i where (:Contrato=-1 or (:Contrato<>-1 and i.sContrato =:Contrato)) and i.IdInsumo =:Insumo ');
          connection.qryUBusca.ParamByName('Contrato').AsString  := global_contrato;
          connection.qryUBusca.ParamByName('Insumo').AsInteger   := materiales.FieldByName('IdInsumo').AsInteger;
          connection.qryUBusca.ParamByName('Almacen').AsInteger  := frmSalidaAlmacen.salida_almacen.FieldByName('IdAlmacen').AsInteger;
          connection.qryUBusca.ParamByName('Item').AsInteger   := materiales.FieldByName('iItem').AsInteger;
        end
        else
        begin
        ...
        connection.qryUBusca2.Params.ParamByName('Folio').AsString        := frmSalidaAlmacen.salida_almacen.FieldByName('iFolioSalida').AsString ;
        connection.qryUBusca2.Params.ParamByName('Insumo').AsInteger      := Materiales.FieldByName('IdInsumo').AsInteger;
        connection.qryUBusca2.Open ;
        ...
        begin
            ...
            BEGIN
                ...
                connection.zUCommand.Params.ParamByName('Insumo').AsInteger  := materiales.FieldValues['IdInsumo'];
                connection.zUCommand.Params.ParamByName('Cantidad').AsFloat  := edtCantidad.Value ;
                connection.zUCommand.Params.ParamByName('Almacen').AsInteger:=frmSalidaAlmacen.salida_almacen.FieldByName('IdAlmacen').AsInteger;
            END
            else begin
                ...
                connection.zUCommand.Params.ParamByName('Folio').AsInteger   := frmSalidaAlmacen.salida_almacen.FieldValues['iFolioSalida'] ;
                connection.zUCommand.Params.ParamByName('FechaS').AsDate     := frmSalidaAlmacen.salida_almacen.FieldValues['dFechaSalida'];
                if (frmSalidaAlmacen.salida_almacen.FieldByName('CodigoSalida').AsString = 'SALA') or (frmSalidaAlmacen.salida_almacen.FieldByName('CodigoSalida').AsString = 'AJIN')then
                   connection.zUCommand.Params.ParamByName('FolioReq').AsInteger := 0
                 else
                   connection.zUCommand.Params.ParamByName('FolioReq').AsInteger  := frmSalidaAlmacen.salida_almacen.FieldValues['iFolioRequisicion'];
                connection.zUCommand.Params.ParamByName('Insumo').AsInteger  := materiales.FieldValues['IdInsumo'];
                connection.zUCommand.Params.ParamByName('Cantidad').AsFloat  := edtCantidad.Value ;
                connection.zUCommand.Params.ParamByName('Almacen').AsInteger:=frmSalidaAlmacen.salida_almacen.FieldByName('IdAlmacen').AsInteger;
            end;
        ...
        begin
            if (frmSalidaAlmacen.salida_almacen.FieldByName('iFolioRequisicion').AsInteger = 0) or (frmSalidaAlmacen.salida_almacen.FieldByName('iFolioRequisicion').AsInteger = null)  then
            begin
...
procedure TfrmAgregarMasivoOP.btnAceptarClick(Sender: TObject);
begin
...
procedure TfrmAgregarMasivoOP.btnAddClick(Sender: TObject);
begin
  if not assigned(zFamilia) then
  begin
    AsignarSQL(zFamilia,'familias_e',pUpdate);
    zFamilia.Open;
  end;
  if not assigned(zSubFamilia) then
  begin
  AsignarSQL(zSubFamilia,'alm_subfamilia',pUpdate);
  zSubFamilia.Open;
  end;
  if not assigned(uMedidas) then
  begin
  ...
  if not assigned(zproducto) then
  begin
...
procedure TfrmAgregarMasivoOP.btnDetalleClick(Sender: TObject);
begin
  ...
  begin
    zExistencia.Active := False;
    AsignarSql(zExistencia, 'consulta_existencia_cc', pUpdate);
    FiltrarDataSet(zExistencia, 'Contrato, IdInsumo', [global_contrato, materiales.FieldByName('IdInsumo').AsString]);
    zExistencia.Open;
  end;
end;

procedure TfrmAgregarMasivoOP.btnExpandirClick(Sender: TObject);
begin
...
function TfrmAgregarMasivoOP.camposLlenos() : Boolean;
var
  ban: boolean;
begin
  ban:=True;
...
procedure TfrmAgregarMasivoOP.cbFamiliaPropertiesEditValueChanged(
  Sender: TObject);
begin
...
procedure TfrmAgregarMasivoOP.cxActualizaClick(Sender: TObject);
var
...
begin
    ...
    begin
        ...
        begin
           ...
           if not camposLlenos() then begin
             ...
             if (Connection.QryUBusca.ParamByName('IdTipoRecurso').AsInteger = 3) and (cbAdicional.Text ='10') then
               ...
               if cbAdicional.Text = '' then
               Connection.QryUBusca.ParamByName('Adicional').Clear
               else
               Connection.QryUBusca.ParamByName('Adicional').AsString:= cbAdicional.Text;
             end;

             if cbFinanciamiento.Text = '' then
               Connection.QryUBusca.ParamByName('Financiamiento').Clear
               else
        ...
        begin
           //if frmSalidaAlmacen.salida_almacen.FieldByName('iFolioRequisicion').AsInteger > 0 then
             if edtCantidad.Value <= 0 then
             begin
                 messageDLG('No se puede dar salida con una cantidad negativa', mtInformation, [mbOk],0);
                 exit;
             end;

             if edtCantidad.Value > materiales.FieldByName('dExistencia').AsFloat then
             begin
                 messageDLG('No se puede dar salida a una cantidad mayor a la existencia', mtInformation, [mbOk],0);
                 exit;
             end;
           addSalida;
           addAsignacion;
    ...
    begin
      ...
      begin
         ...
         if frmEntradaAlmacen.almacen_entrada.FieldByName('IdTipo').AsInteger <> 12 then
         BEGIN
            materiales.Refresh;
            if connection.uContrato.FieldByName('FK_Titulo').AsString='SUBSEA 7' then
            BEGIN
         ...
         END;
         frmEntradaAlmacen.pEntradas.Refresh;
         frmEntradaAlmacen.almacen_entrada.Refresh;
         frmEntradaAlmacen.PgControl.Enabled:=True;
         frmEntradaAlmacen.Grid_Entradas.Enabled := True;
      end;
...
procedure TfrmAgregarMasivoOP.cxGuardiasPropertiesChange(Sender: TObject);
begin
  if global_frmActivo = 'Entrada_Almacen' then
  begin
    View_Grid_MaterialesFechaCosto.Visible:=False;
...
procedure TfrmAgregarMasivoOP.cxPageCostosChange(Sender: TObject);
begin
  ...
  if cxPageCostos.ActivePage =  cxTabProyectos then
  begin
    zExistencia.Active := False;
    AsignarSql(zExistencia, 'consulta_existencia_cc', pUpdate);
    FiltrarDataSet(zExistencia, 'Contrato, IdInsumo', [global_contrato, materiales.FieldByName('IdInsumo').AsString]);
...
procedure TfrmAgregarMasivoOP.cxTabCosto_OCShow(Sender: TObject);
begin
  if materiales.Active then
    ScrollMateriales;
end;

procedure TfrmAgregarMasivoOP.cxTabcosto_PUShow(Sender: TObject);
begin
  if materiales.Active then
    ScrollMateriales;
end;

procedure TfrmAgregarMasivoOP.dtbCancelarClick(Sender: TObject);
begin
 Close;
end;
...
procedure TfrmAgregarMasivoOP.FormClose(Sender: TObject;
  var Action: TCloseAction);
var
   i :integer;
begin
   for I := 0 to global_FormNoilPrincipal.MDIChildCount - 1 do
       global_FormNoilPrincipal.MDIChildren[i].Enabled := True;
end;

procedure TfrmAgregarMasivoOP.FormShow(Sender: TObject);
begin
  id:='-1';
...
//    if(assigned(frmAvisoEmbarque)) then begin
      if ((connection.uContrato.FieldByName('FK_Titulo').AsString='CMMI') and (frmSalidaAlmacen.salida_almacen.FieldByName('CodigoSalida').AsString = 'AJS')) or
         ((connection.uContrato.FieldByName('FK_Titulo').AsString='CMMI') and (frmSalidaAlmacen.salida_almacen.FieldByName('CodigoSalida').AsString = 'EPP')) or
         ((connection.uContrato.FieldByName('FK_Titulo').AsString='CMMI') and (frmSalidaAlmacen.salida_almacen.FieldByName('CodigoSalida').AsString = 'SALA')) or
         ((connection.uContrato.FieldByName('FK_Titulo').AsString='CMMI') and (frmSalidaAlmacen.salida_almacen.FieldByName('CodigoSalida').AsString = 'SAD')) then
      BEGIN
          AsignarSQL(materiales,'catalogo_agregarmasivo_cc',pUpdate);
          FiltrarDataSet(materiales,'Contrato,Usuario,Codigo,sNumeroOrdenCC',
          [global_contrato,Global_Usuario,-1, frmSalidaAlmacen.Grid_EntradasDBTableView1.DataController.DataSet.FieldByName('sNumeroOrdenCC').AsString ])  ;
      END

    ELSE
    if (connection.uContrato.FieldByName('FK_Titulo').AsString='SIANI') then  BEGIN
      if not(frmSalidaAlmacen.salida_almacen.FieldByName('iFolioRequisicion').IsNull)  then
      begin
       noAplica:=True;
       AsignarSQL(materiales,'OC_lista_materiales_req',pUpdate);
       FiltrarDataSet(materiales,'Contrato,FolioReq',[global_contrato,frmSalidaAlmacen.salida_almacen.FieldValues['iFolioRequisicion']]);
      end
      else
      begin
         noAplica:=False;
    ...
    END
    else if (frmSalidaAlmacen.salida_almacen.FieldByName('CodigoSalida').AsString = 'SOLM') then begin
        ...
        [global_contrato,Global_Usuario,frmSalidaAlmacen.salida_almacen.FieldByName('IdAlmacen').AsInteger, frmSalidaAlmacen.salida_almacen.FieldByName('iFolioSolicitud').AsInteger]);
    end
    else if (frmSalidaAlmacen.salida_almacen.FieldByName('CodigoSalida').AsString = 'TRAS') and
        ...
        FiltrarDataSet(materiales,'Contrato,Usuario,Almacen',
        [global_contrato,Global_Usuario,frmSalidaAlmacen.salida_almacen.FieldByName('IdAlmacen').AsInteger]);
    end
    else if (frmSalidaAlmacen.salida_almacen.FieldByName('CodigoSalida').AsString = 'SALA') and
             (connection.uContrato.FieldByName('FK_Titulo').AsString='SUBSEA 7') then begin
        ...
        [global_contrato,Global_Usuario,frmSalidaAlmacen.salida_almacen.FieldByName('IdAlmacen').AsInteger, frmSalidaAlmacen.salida_almacen.FieldByName('iFolioEntrada').AsInteger]);
    end
    ELSE
    begin
        cxLabelOrigen.Visible := True;
        cxLabelOrigen.Caption := '('+frmSalidaAlmacen.cxAlmacen.Text+')';
        AsignarSQL(materiales,'catalogo_agregarmasivo_salida',pUpdate);
        FiltrarDataSet(materiales,'Contrato,Usuario,Almacen,FolioReq, sNumeroOrdenCC',
        [global_contrato,Global_Usuario,frmSalidaAlmacen.salida_almacen.FieldByName('IdAlmacen').AsInteger, frmSalidaAlmacen.salida_almacen.FieldByName('iFolioRequisicion').AsInteger, frmSalidaAlmacen.Grid_EntradasDBTableView1.DataController.DataSet.FieldByName('sNumeroOrdenCC').AsString]);
    end;
    materiales.Open;
    if (connection.uContrato.FieldByName('FK_Titulo').AsString='SUBSEA 7') then
  ...
  END
    ...
    if global_frmActivo = 'Entrada_Almacen' then
    begin
      cxLabelOrigen.Caption := '('+frmEntradaAlmacen.cxAlmacen.Text+')';
      selectEntradas;
    end
    else
   ...
   if  global_frmActivo = 'Solicitud' then
    begin
       materiales.Active := False;
      AsignarSQL(materiales,'catalogo_masivo_solicitud',pUpdate);
      FiltrarDataSet(materiales,'Usuario,Codigo,Contrato, sNumeroOrdenCC',[Global_Usuario,'-1',global_contrato, FrmSolicitudMaterial.cxView_Requsicion.DataController.DataSet.FieldByName('sNumeroOrdenCentroC').AsString]);
      materiales.Open;
    end
    else
    begin
...
//    end;
  ...
  begin
    zRecurso.Active:=False;
    zRecurso.SQL.Text:='select IdTiporecurso, Nombre from op_tiporecurso union Select -1 ,'+QuotedStr('TODOS')+'  as Nombre';
    zRecurso.Open;
  end;
  ...
  begin
    ...
    if (global_frmActivo = 'Salida_Almacen') or (global_frmActivo = 'Aviso_Embarque')then
  begin
  ...
  if (global_frmActivo = 'Solicitud') then
  begin
     View_Grid_MaterialesCentroCosto.Visible := True;
     View_Grid_MaterialesTrazabilidad.Visible := False;
     View_Grid_MaterialesCosto.Visible := False;
     View_Grid_MaterialesMoneda.Visible := False;
  end;
...
procedure TfrmAgregarMasivoOP.frmBarra1btnExitClick(Sender: TObject);
begin
   Close;
end;

procedure TfrmAgregarMasivoOP.frmBarraH11btnEditClick(Sender: TObject);
begin
    if not assigned(zFamilia) then
  begin
    AsignarSQL(zFamilia,'familias_e',pUpdate);
...
procedure TfrmAgregarMasivoOP.ScrollMateriales;
begin
...
procedure TfrmAgregarMasivoOP.SelectMaterial;
begin
...
procedure TfrmAgregarMasivoOP.View_Grid_MaterialesCellClick(
...
procedure TfrmAgregarMasivoOP.View_Grid_MaterialesCellDblClick(
  Sender: TcxCustomGridTableView; ACellViewInfo: TcxGridTableDataCellViewInfo;
  AButton: TMouseButton; AShift: TShiftState; var AHandled: Boolean);
begin
//Esta parte de codigo la comento ya que ahora la existencia por centro de costo es en la parte de detalle : 09-enero-2020
//Para que se pueda con el dblclic ampliar descripcion del material.
...
procedure TfrmAgregarMasivoOP.View_Grid_MaterialesKeyUp(Sender: TObject;
