...
type
  TfrmOrdenTrabajo = class(TForm)
    ...
    clCentroCosto: TcxDBLookupComboBox;
    dxLayoutItem6: TdxLayoutItem;
    dxLayoutItem7: TdxLayoutItem;
    cxFindCC: TcxButton;
    dsCentroCosto: TDataSource;
    zCentroCosto: TUniQuery;
    OrdenesdeTrabajoOrdenCentroc: TStringField;
...
//    procedure AsginaFrenteUsuario(dParamFrente: string);
    ...
    procedure cxFindCCClick(Sender: TObject);
...
uses
  ...
  frm_Presupuesto, frm_GridFM;

{$R *.dfm}

procedure TfrmOrdenTrabajo.GlobalKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TfrmOrdenTrabajo.EnterControl(Sender: TObject);
begin
...
procedure TfrmOrdenTrabajo.SalidaControl(Sender: TObject);
begin
    if (sender is tcxDBTextEdit) then
...
begin
   guardar_leer_grid(cxVistaOrdenes,frmOrdenTrabajo,1);
    action := cafree ;
end;
...
begin
    ...
    try
       ...
       {
        CUANDO SE AGREGUE NUEVO CAMPO A LA TABLA OrdenesdeTrabajo,
        EDITAR <<FIELDS  EDITOR>> DEL UNIQUERY OrdenesdeTrabajo PARA QUE TAMBIÉN SE AÑADA
       }
       AsignarSQL(OrdenesdeTrabajo,'lista_presupuesto', pUpdate);
       FiltrarDataSet(OrdenesdeTrabajo, 'Contrato, TipoProyecto', [Global_contrato, 'ORDEN_TRABAJO']);
       ...
       FiltrarDataSet(Estatus,'E1, E2, E3, E4, E5, E6, E7',['A', 'C', 'PE', 'SE', 'T', 'NA', 'NA']);
       Estatus.Open;
       ...
       PermisosBotones(frmOrdenTrabajo,btnPermisos);
  ...
  guardar_leer_grid(cxVistaOrdenes,frmOrdenTrabajo,0);
end;

procedure TfrmOrdenTrabajo.grid_ordenesEnter(Sender: TObject);
begin
...
procedure TfrmOrdenTrabajo.grid_ordenesKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  If frmBarraH21.btnCancel.Enabled = True then
      frmBarraH21.btnCancel.Click ;
...
procedure TfrmOrdenTrabajo.grid_ordenesKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  If frmBarraH21.btnCancel.Enabled = True then
      frmBarraH21.btnCancel.Click ;
...
procedure TfrmOrdenTrabajo.grid_ordenesCellClick(Column: TColumn);
begin
  If frmBarraH21.btnCancel.Enabled = True then
      frmBarraH21.btnCancel.Click ;
...
procedure TfrmOrdenTrabajo.frmBarra1btnRefreshClick(Sender: TObject);
begin
 try
...
procedure TfrmOrdenTrabajo.Insertar1Click(Sender: TObject);
begin
    frmBarraH11.btnAdd.click
end;

procedure TfrmOrdenTrabajo.OrdenesdeTrabajoCalcFields(DataSet: TDataSet);
var
   dDiaSiguiente :tDate;
begin
...
procedure TfrmOrdenTrabajo.OrdenesdeTrabajoiConsecutivoSetText(Sender: TField;
  const Text: string);
begin
  sender.Value:=abs(StrToIntDef(text,0));
end;

procedure TfrmOrdenTrabajo.OrdenesdeTrabajoiConsecutivoTierraSetText(Sender: TField;
  const Text: string);
begin
  sender.Value:=abs(StrToIntDef(text,0));
end;

procedure TfrmOrdenTrabajo.OrdenesdeTrabajoiJornadaSetText(Sender: TField;
  const Text: string);
begin
  sender.Value:=abs(StrToIntDef(text,0));
end;

procedure TfrmOrdenTrabajo.Editar1Click(Sender: TObject);
begin
    frmBarraH11.btnEdit.Click
end;

procedure TfrmOrdenTrabajo.Registrar1Click(Sender: TObject);
begin
    frmBarraH21.btnPost.Click
end;

procedure TfrmOrdenTrabajo.Can1Click(Sender: TObject);
begin
    frmBarraH21.btnCancel.Click
end;

procedure TfrmOrdenTrabajo.cxButton1Click(Sender: TObject);
begin
...
procedure TfrmOrdenTrabajo.cxButton2Click(Sender: TObject);
begin
...
procedure TfrmOrdenTrabajo.cxFindCCClick(Sender: TObject);
begin
  ...
  frmGridFM.cxGrid1DBTableView1.DataController.CreateItemByField(clCentroCosto.Properties.ListColumns[0].Field);
  frmGridFM.cxGrid1DBTableView1.Columns[0].Caption := 'Código';
  frmGridFM.cxGrid1DBTableView1.DataController.CreateItemByField(clCentroCosto.Properties.ListColumns[1].Field);
  ...
  if frmGridFM.ShowModal = mrOk then
      begin
      clCentroCosto.EditValue:=frmGridFM.cxGrid1DBTableView1.DataController.DataSource.DataSet.FieldByName('sNumeroOrden').Value;
      end;
end;

procedure TfrmOrdenTrabajo.cxVistaOrdenesCellClick(
...
procedure TfrmOrdenTrabajo.cxVistaOrdenesDblClick(Sender: TObject);
begin
...
procedure TfrmOrdenTrabajo.cxVistaOrdenesStylesGetContentStyle(
  Sender: TcxCustomGridTableView; ARecord: TcxCustomGridRecord;
  AItem: TcxCustomGridTableItem; var AStyle: TcxStyle);
  var
...
procedure TfrmOrdenTrabajo.Eliminar1Click(Sender: TObject);
begin
    frmBarraH11.btnDelete.Click
end;

function TfrmOrdenTrabajo.tablasDependientes(idOrig: string): boolean;
var
...
function TfrmOrdenTrabajo.posibleBorrar(idOrig: string): boolean;
var
...
procedure TfrmOrdenTrabajo.ActualizaReporte(sFrente: string; valor :boolean);
...
procedure TfrmOrdenTrabajo.btnAddClick(Sender: TObject);
begin
  try
   Opcion := 'Nuevo';
   zConvenios.Filtered := False;
...
//   frmBarraH21.btnActive;
   ...
   OrdenesdeTrabajo.FieldByName('TipoProyecto').AsString := 'ORDEN_TRABAJO';
   ...
   OrdenesdeTrabajo.FieldByName('OrdenCentroC').AsString := zCentroCosto.FieldByName('sNumeroOrden').AsString;
   sIdFolio.SetFocus;
...
procedure TfrmOrdenTrabajo.btnCancelClick(Sender: TObject);
begin
 try
...
procedure TfrmOrdenTrabajo.btnDeleteClick(Sender: TObject);
var
  cadena: string;
begin
...
procedure TfrmOrdenTrabajo.btnDetalleClick(Sender: TObject);
begin
...
procedure TfrmOrdenTrabajo.btnEditClick(Sender: TObject);
begin
...
procedure TfrmOrdenTrabajo.btnExportaClick(Sender: TObject);
var
  rutaArchivo: string;
  Excel, Sheet: Variant;
  dlgSave : TSaveDialog;
...
procedure TfrmOrdenTrabajo.btnImpInformeClick(Sender: TObject);
begin
...
procedure TfrmOrdenTrabajo.btnPernoctaClick(Sender: TObject);
begin
...
procedure TfrmOrdenTrabajo.btnPlataformasClick(Sender: TObject);
begin
...
procedure TfrmOrdenTrabajo.btnPostClick(Sender: TObject);
var
   cadena   : string;
   nombres,
   cadenas  : TStringList;
...
procedure TfrmOrdenTrabajo.btnPrinterClick(Sender: TObject);
begin
...
procedure TfrmOrdenTrabajo.btnRefreshClick(Sender: TObject);
begin
   OrdenesdeTrabajo.Refresh;
end;
