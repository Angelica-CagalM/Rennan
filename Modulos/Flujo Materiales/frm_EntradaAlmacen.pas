...
uses
  ...
  dxCore, CxGridExportLink, ShellApi,cxDateUtils;
...
uses frm_connection, UnitValidaTexto, Func_Genericas, frm_firmasNota, unitMetodos,
  frm_repositorio,frm_almacenamiento, frm_agregar_masivoOP;
...
begin
  guardar_leer_grid(Grid_ViewEntradas,frmEntradaAlmacen,1);
  guardar_leer_grid(cxViewMaterialEntrada,frmEntradaAlmacen,1);
  ListaFiles.Clear;
  ListaFiles.destroy;
...
begin
  ...
  try
    ...
    BEGIN
      dxLayoutAutoCreatedGroup9.Visible := True;
      FechaInicio.Date:= now;
      FechaFin.date := now;
    END
    ELSE
      dxLayoutAutoCreatedGroup9.Visible := FALSE;
  ...
  guardar_leer_grid(Grid_ViewEntradas,frmEntradaAlmacen,0);
  guardar_leer_grid(cxViewMaterialEntrada,frmEntradaAlmacen,0);
end;
...
procedure TfrmEntradaAlmacen.btnExportaClick(Sender: TObject);
var
  rutaArchivo: string;
  Excel, Sheet: Variant;
  dlgSave : TSaveDialog;
begin
  ...
  dlgSave.FileName := 'Entradas de Almacen';
...
procedure TfrmEntradaAlmacen.btnImpInformeClick(Sender: TObject);
var
  entradasGrid : String;
...
procedure TfrmEntradaAlmacen.btnPostClick(Sender: TObject);
begin
   pEntradas.Post;
   pEntradas.Refresh;
...
procedure TfrmEntradaAlmacen.btnRutaClick(Sender: TObject);
var
  BlobStream : TStream;
...
begin
  ...
  PermisosBotones(frmEntradaAlmacen,btnPermisos);
end;
...
procedure TfrmEntradaAlmacen.frmBarraH22btnCancelClick(Sender: TObject);
begin
...
procedure TfrmEntradaAlmacen.frmBarraH22btnPostClick(Sender: TObject);
var
...
begin
   ...
   begin
      ...
      //Filtrar solo los que no han entrado al 100%
        FolioCompra.Filtered := False;
        FolioCompra.Filter   := 'TotalOC > TotalEntrada ';
        FolioCompra.Filtered := True;
...
procedure TfrmEntradaAlmacen.cxAlmacenamientoClick(Sender: TObject);
begin
...
procedure TfrmEntradaAlmacen.cxComprasxProveedorClick(Sender: TObject);
var
...
procedure TfrmEntradaAlmacen.cxEditaDetalleClick(Sender: TObject);
begin
     If almacen_entrada.RecordCount > 0 Then
     Begin
         if almacen_entrada.FieldValues['sStatus'] <> 'PENDIENTE' then
...
procedure TfrmEntradaAlmacen.cxViewMaterialEntradaCellClick(
...
procedure TfrmEntradaAlmacen.cxViewMaterialEntradaCellDblClick(
  Sender: TcxCustomGridTableView; ACellViewInfo: TcxGridTableDataCellViewInfo;
  AButton: TMouseButton; AShift: TShiftState; var AHandled: Boolean);
begin
...
procedure TfrmEntradaAlmacen.cxNuevoDetalleClick(Sender: TObject);
var
   i : integer;
begin
    If (almacen_entrada.RecordCount > 0) Then
    Begin
       ...
       begin
         ...
         if uppercase(global_FormNoilPrincipal.MDIChildren[i].Name) = uppercase('frmEntradaAlmacen') then
...
procedure TfrmEntradaAlmacen.almacen_entradaAfterOpen(DataSet: TDataSet);
begin
    almacen_entrada.Cancel;
end;

procedure TfrmEntradaAlmacen.btnDetalleClick(Sender: TObject);
begin
//  if connection.uContrato.FieldByName('FK_Titulo').AsString<>'CMMI' then
...
procedure TfrmEntradaAlmacen.EnterControl(Sender: TObject);
begin
...
procedure TfrmEntradaAlmacen.SalidaControl(Sender: TObject);
begin
    if (sender is tcxDBTextEdit) then
...
procedure TfrmEntradaAlmacen.GlobalKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TfrmEntradaAlmacen.Grid_ViewEntradasCellClick(
  Sender: TcxCustomGridTableView; ACellViewInfo: TcxGridTableDataCellViewInfo;
  AButton: TMouseButton; AShift: TShiftState; var AHandled: Boolean);
begin
...
procedure TfrmEntradaAlmacen.Grid_ViewEntradasKeyUp(Sender: TObject;
...
procedure TfrmEntradaAlmacen.entrada_scroll;
begin
    if almacen_entrada.RecordCount > 0 then
    begin
