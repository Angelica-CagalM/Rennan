...
uses
  ...
  dxToggleSwitch, CxGridExportLink, ShellApi, dxDBToggleSwitch;
...
type
  TfrmSalidaAlmacen = class(TForm)
    ...
    cpPersonal: TcxDBLookupComboBox;
    ...
    cbCentroCosto: TcxDBLookupComboBox;
    ...
    procedure cpPersonalPropertiesEditValueChanged(Sender: TObject);
...
uses frm_connection , Func_Genericas, frm_agregar_masivoOP, unitMetodos, frm_firmasNota;
...
begin
guardar_leer_grid(Grid_EntradasDBTableView1,frmSalidaAlmacen,1);
guardar_leer_grid(GridPartidasView,frmSalidaAlmacen,1);
  action := cafree ;
end;
...
begin
    ...
    if connection.uContrato.FieldByName('FK_Titulo').AsString='CMMI' then  begin
      ...
      AsignarSQL(zCentroCosto,'ordenesdetrabajo_req',pUpdate);
      FiltrarDataSet(zCentroCosto,'Contrato,Tipo',[global_contrato,'CENTRO_COSTO']);
      zCentroCosto.Open;
    end;
    ...
    BEGIN
      dxLayoutAutoCreatedGroup14.Visible := True;
      FechaInicio.Date := now;
      fechaFin.Date  := now;
    END
    else
      dxLayoutAutoCreatedGroup14.Visible:= False;

    qPersonal.Active:= False;
    AsignarSQL(qPersonal,'master_personal_salida',pUpdate);
    FiltrarDataSet(qPersonal,'Empresa,Activo',[ Connection.ucontrato.FieldByName('IdEmpresa').AsInteger ,'Si' ]);
    qPersonal.Open;

    zEmbarcacion.Active:=False;
    AsignarSQL(zEmbarcacion,'embarcaciones_salidas', pUpdate );
    FiltrarDataSET(zEmbarcacion,'usuario',[global_usuario]);
    zEmbarcacion.Open;
    ...
    PermisosBotones(frmSalidaAlmacen,btnPermisos);
...
//       cxSolicitudEmb.Visible:=False;
     ...
     dxLayoutItem24.Visible:=True;
    guardar_leer_grid(Grid_EntradasDBTableView1,frmSalidaAlmacen,0);
    guardar_leer_grid(GridPartidasView,frmSalidaAlmacen,0);
end;
...
procedure TfrmSalidaAlmacen.btnExportaClick(Sender: TObject);
var
  rutaArchivo: string;
  Excel, Sheet: Variant;
  dlgSave : TSaveDialog;
...
procedure TfrmSalidaAlmacen.btnImagenClick(Sender: TObject);
var
  BlobStream : TStream;
...
procedure TfrmSalidaAlmacen.btnImpInformeClick(Sender: TObject);
var
  salidasGrid : String;
...
procedure TfrmSalidaAlmacen.btnPostClick(Sender: TObject);
var
...
begin
  ...
  if connection.uContrato.FieldByName('FK_Titulo').AsString = 'CMMI' then
    if salida_almacen.FieldByName('IdTipo').AsInteger = 4 then
      salida_almacen.FieldByName('iFolioRequisicion').Clear;
  ...
  Begin
      ...
      cadenas.Add(cbCentroCosto.Text);
...
procedure TfrmSalidaAlmacen.btnPrinterClick(Sender: TObject);
var
...
procedure TfrmSalidaAlmacen.btnRefreshClick(Sender: TObject);
begin
   SavePlace := Grid_EntradasDBTableView1.DataController.DataSource.DataSet.GetBookmark;
...
procedure TfrmSalidaAlmacen.GridPartidasViewCellDblClick(
  Sender: TcxCustomGridTableView; ACellViewInfo: TcxGridTableDataCellViewInfo;
  AButton: TMouseButton; AShift: TShiftState; var AHandled: Boolean);
begin
...
procedure TfrmSalidaAlmacen.GridPartidasViewMouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
if Connection.uContrato.FieldByName('FK_Titulo').AsString = 'CMMI' then
  begin
...
procedure TfrmSalidaAlmacen.Grid_EntradasDBTableView1CellDblClick(
  Sender: TcxCustomGridTableView; ACellViewInfo: TcxGridTableDataCellViewInfo;
  AButton: TMouseButton; AShift: TShiftState; var AHandled: Boolean);
begin
...
procedure TfrmSalidaAlmacen.Grid_EntradasDBTableView1DataControllerFilterBeforeChange(
  Sender: TcxDBDataFilterCriteria; ADataSet: TDataSet;
  const AFilterText: string);
begin
  IsOpen:=False;
...
procedure TfrmSalidaAlmacen.Grid_EntradasDBTableView1KeyUp(Sender: TObject;
...
procedure TfrmSalidaAlmacen.Grid_EntradasDBTableView1MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TfrmSalidaAlmacen.InsertarNumeroMaterial1Click(Sender: TObject);
var
...
procedure TfrmSalidaAlmacen.LimpiarCdigoMaterial1Click(Sender: TObject);
var
    zActualiza : tUniquery;
    indice, iGrid    : integer;
    lContinua : boolean;
...
procedure TfrmSalidaAlmacen.PopupPrincipalPopup(Sender: TObject);
begin
  if Connection.uContrato.FieldByName('FK_Titulo').AsString = 'CMMI' then
    begin
...
//         AsignarCdigoMaterial1.Enabled := True;
     ...
     if salida_almacen.FieldByName('IdTipo').AsInteger = 14 then
     begin
        Insertar1.Enabled := False;
        Editar1.Enabled := False;
        Eliminar1.Enabled := False;
        InsertarNumeroMaterial1.Enabled := False;
        LimpiarCdigoMaterial1.Enabled := False;
     end;
    end;
end;
...
procedure TfrmSalidaAlmacen.cpPersonalPropertiesEditValueChanged(
  Sender: TObject);
begin
  if (QPersonal.Active )then
  if cpPersonal.EditValue <> null then
  begin
    if QPersonal.Locate('IdPersonal',cpPersonal.EditValue,[]) then
      if salida_almacen.State in [dsInsert,dsEdit] then
        salida_Almacen.FieldByName('sNombre').AsString := QPersonal.FieldByName('NombreCompleto').AsString;
  end;
end;

procedure TfrmSalidaAlmacen.cxButton1Click(Sender: TObject);
var zUpt: TUniquery;
begin
...
procedure TfrmSalidaAlmacen.cxButton2Click(Sender: TObject);
begin
 if True then
...
procedure TfrmSalidaAlmacen.cxButton3Click(Sender: TObject);
begin
...
procedure TfrmSalidaAlmacen.cxCancelaMaterialClick(Sender: TObject);
begin
...
procedure TfrmSalidaAlmacen.cxCancelarDetalleClick(Sender: TObject);
begin
   salida_detalle.Cancel;
...
procedure TfrmSalidaAlmacen.cxComprasxProveedorClick(Sender: TObject);
var
  frxOrdenCompra : TfrxDBDataset;
begin
  try
      frmRepositorio.Reporte.Filtered := False;
      AsignarSQL(frmRepositorio.Reporte,'Reporte_Costos_Salida',pUpdate);
      FiltrarDataSet(frmRepositorio.Reporte,'Contrato',[global_contrato]);
      frmRepositorio.Reporte.Open;

      frmRepositorio.Reporte.Filter   := ' (FechaSalida >='+QuotedStr(FechaSQL(FechaInicio.Date))+' and FechaSalida <='+QuotedStr(FechaSQL(FechaFin.Date))+')';;
      ...
      frxEntrada.LoadFromFile(global_files + global_miReporte + '_ALM_ReporteCostoSalidas.fr3');
      frxentrada.ShowReport();
      frmRepositorio.Reporte.Filtered := False;
    except
      on e: exception do begin
...
procedure TfrmSalidaAlmacen.cxEditaDetalleClick(Sender: TObject);
begin
 If salida_almacen.RecordCount > 0 Then
     Begin
...
procedure TfrmSalidaAlmacen.cxEliminarDetalleClick(Sender: TObject);
var
  Id : Integer;
begin
     If salida_detalle.RecordCount > 0 Then
...
procedure TfrmSalidaAlmacen.cxEmpleadoPropertiesEditValueChanged(
  Sender: TObject);
begin
...
procedure TfrmSalidaAlmacen.cxGuardarDetalleClick(Sender: TObject);
begin
    salida_detalle.Post;
    cxCancelarDetalle.Click;
end;

procedure TfrmSalidaAlmacen.cxNuevoDetalleClick(Sender: TObject);
var
  i : integer;
begin
    If (salida_almacen.RecordCount > 0) Then
     Begin
          ...
          begin
             ...
             if uppercase(global_FormNoilPrincipal.MDIChildren[i].Name) = uppercase('frmSalidaAlmacen') then
...
procedure TfrmSalidaAlmacen.cxPageDetalleClick(Sender: TObject);
begin
 try
...
procedure TfrmSalidaAlmacen.cxRequisicionPropertiesChange(Sender: TObject);
begin
...
procedure TfrmSalidaAlmacen.cxSalidaEPPClick(Sender: TObject);
var
frxDBEntrada : TfrxDBDataset;
begin
  try
...
procedure TfrmSalidaAlmacen.cxSalidaResguardoClick(Sender: TObject);
var
frxDBEntrada : TfrxDBDataset;
begin
  try
...
procedure TfrmSalidaAlmacen.cxSolicitudPropertiesChange(Sender: TObject);
begin
...
procedure TfrmSalidaAlmacen.cxTabSheet3Show(Sender: TObject);
begin
  opcionImprimir.EditValue := 1;
end;

procedure TfrmSalidaAlmacen.cxTipoSalidaExit(Sender: TObject);
begin
   cxTipoSalida.Style.Color := global_color_SalidaERP;
    if connection.uContrato.FieldByName('FK_Titulo').AsString = 'CMMI' then begin
      if (Salida_almacen.State = dsInsert) then
        Salida_almacen.FieldByName('sFolioSalida').AsString  := autoFolioAviso(Salida_almacen,'almacen_salida',connection.uContrato.FieldByName('IdEmpresa').AsInteger,0,0,cxTipoSalida.DataBinding.DataSource.DataSet.FieldByName('IdTipo').AsInteger);
    end;
...
//   end;
   ...
   If (cxTipoSalida.Text = 'EQ. DE PROTECCION PERSONAL') and (connection.uContrato.FieldByName('FK_Titulo').AsString='CMMI') then
   begin
    dxLayoutItem38.Visible:=True;
    dxLayoutItem10.Visible := False;
    if salida_Almacen.State in [dsInsert, dsEdit] then
      Salida_Almacen.FieldByName('iFolioRequisicion').AsVariant := null;
...
procedure TfrmSalidaAlmacen.cxTipoSalidaPropertiesChange(Sender: TObject);
begin
  if insert then
  begin
    cxProyecto.SetFocus;
...
procedure TfrmSalidaAlmacen.AsignacionPersonal;
var
...
procedure TfrmSalidaAlmacen.btnAddClick(Sender: TObject);
begin
...
procedure TfrmSalidaAlmacen.btnCancelClick(Sender: TObject);
begin
  insert:=False;
...
procedure TfrmSalidaAlmacen.btnDeleteClick(Sender: TObject);
begin
  If salida_almacen.RecordCount > 0 Then
...
procedure TfrmSalidaAlmacen.btnDetalleClick(Sender: TObject);
begin
  panelDown.Visible:= False;
...
procedure TfrmSalidaAlmacen.btnEditClick(Sender: TObject);
begin
    if salida_almacen.FieldValues['sStatus'] <> 'PENDIENTE' then
    begin
       messageDLG('La Salida de Material se encuentra en estatus '+salida_almacen.FieldValues['sStatus'], mtInformation, [mbOk], 0);
...
procedure TfrmSalidaAlmacen.EnterControl(Sender: TObject);
begin
...
procedure TfrmSalidaAlmacen.SalidaControl(Sender: TObject);
begin
    if (sender is tcxDBTextEdit) then
...
procedure TfrmSalidaAlmacen.GlobalKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmSalidaAlmacen.salida_detalleAfterScroll(DataSet: TDataSet);
begin
  IdRequi :=  Salida_Almacen.FieldByName('iFolioRequisicion').AsInteger;
end;

procedure TfrmSalidaAlmacen.salida_scroll;
begin
    if salida_almacen.RecordCount > 0 then
    begin
      ...
      if connection.uContrato.FieldByName('FK_Titulo').AsString='CMMI' then
         GridPartidasViewColumn2.Visible := False;
