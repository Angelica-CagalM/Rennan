...
uses
  ...
  UnitReportes, dxCore, cxDateUtils;

type
  TfrmTransito = class(TForm)
    Panel3: TPanel;
    ...
    cxView_EmbarqueColumn8: TcxGridDBColumn;
    cxView_EmbarqueColumn9: TcxGridDBColumn;
    Panel4: TPanel;
...
procedure TfrmTransito.btnAddClick(Sender: TObject);
begin
//    try
...
procedure TfrmTransito.btnCancelClick(Sender: TObject);
begin
  try
    ...
    PermisosBotones(frmTransito,btnPermisos);
  EXCEPT
  ON e:Exception do
    ShowMessage('Error: ' + e.Message);
...
procedure TfrmTransito.btnDeleteClick(Sender: TObject);
begin
...
procedure TfrmTransito.btnDetalleClick(Sender: TObject);
begin
...
procedure TfrmTransito.btnEditClick(Sender: TObject);
begin
...
procedure TfrmTransito.btnExportaClick(Sender: TObject);
begin
ExportarToExcel(cxGridMateriales);
end;

procedure TfrmTransito.btnImpInformeClick(Sender: TObject);
var
  avisosGrid : String;
...
procedure TfrmTransito.btnLimpiarFClick(Sender: TObject);
begin
//   cxView_Embarque.DataController.Filter.FilterText := '';
//   cxView_Embarque.DataController.Filter.Clear;
//   zAvisosEmbarque.Filtered := False;
    cxView_Embarque.DataController.Filter.FilterText := '';
    cxView_Embarque.DataController.Filter.Clear;
    zAvisosEmbarque.Filtered := False;
    zAvisosEmbarque.Filter   :=' sStatus = '+QuotedStr('VALIDADO');
    zAvisosEmbarque.Filtered := True;
end;

procedure TfrmTransito.btnPostClick(Sender: TObject);
begin
  try
    IsOpen := False;
...
procedure TfrmTransito.btnPrinterClick(Sender: TObject);
var
...
procedure TfrmTransito.btnRefreshClick(Sender: TObject);
begin
    zAvisosEmbarque.Refresh;
    zDetalleMaterial.Refresh;
end;

procedure TfrmTransito.CancelarClick(Sender: TObject);
begin
...
procedure TfrmTransito.cxCancelarDetalleClick(Sender: TObject);
begin
...
procedure TfrmTransito.cxEditaDetalleClick(Sender: TObject);
begin
...
procedure TfrmTransito.cxEliminarDetalleClick(Sender: TObject);
var
  Id : Integer;
begin
     If zDetalleMaterial.RecordCount > 0 Then
...
procedure TfrmTransito.cxFiltroRequeridoClick(Sender: TObject);
begin
//    zAvisosEmbarque.Filtered := False;
//    zAvisosEmbarque.Filter   :='  FechaAviso >= '+QuotedStr(FechaSQL(cxInicioInspeccion.Date))+' and FechaAviso <= '+QuotedStr(FechaSQL(cxFinInspeccion.Date));
//    zAvisosEmbarque.Filtered := True;

if cxOpcionesFiltro.ItemIndex = 0 then
    begin
        zAvisosEmbarque.Filtered := False;
        zAvisosEmbarque.Filter   :=' sStatus = '+QuotedStr(cxOpcionesFiltro.Text)+ ' and FechaAviso >= '+QuotedStr(FechaSQL(cxInicioInspeccion.Date))+' and FechaAviso <= '+QuotedStr(FechaSQL(cxFinInspeccion.Date));
        zAvisosEmbarque.Filtered := True;
    end;
...
procedure TfrmTransito.cxGuardarDetalleClick(Sender: TObject);
begin
    zDetalleMaterial.Post;
    cxCancelarDetalle.Click;
end;

procedure TfrmTransito.cxNuevoDetalleClick(Sender: TObject);
begin
  tdCantidad.Text  := '';
    If (zAvisosEmbarque.RecordCount > 0) Then
     Begin
...
procedure TfrmTransito.cxSalidaPropertiesChange(Sender: TObject);
begin
...
procedure TfrmTransito.cxTabSheet3Show(Sender: TObject);
begin
  OpcionImprimir.EditValue := 1;
end;

procedure TfrmTransito.cxView_EmbarqueCellClick(
...
procedure TfrmTransito.cxView_MaterialesCellDblClick(
  Sender: TcxCustomGridTableView; ACellViewInfo: TcxGridTableDataCellViewInfo;
  AButton: TMouseButton; AShift: TShiftState; var AHandled: Boolean);
begin
...
procedure TfrmTransito.FormClose(Sender: TObject;
...
begin
  try
   ...
   guardar_leer_grid(cxView_Embarque,frmTransito,1);
   guardar_leer_grid(cxView_Materiales,frmTransito,1);
  action := caFree
end;

procedure TfrmTransito.FormShow(Sender: TObject);
var
temE:TUniQuery;
begin
...
//    zOrdenesTrabajo.Open ;
    ...
    guardar_leer_grid(cxView_Embarque,frmTransito,0);
    guardar_leer_grid(cxView_Materiales,frmTransito,0);
end;

procedure TfrmTransito.SalvarClick(Sender: TObject);
var
...
procedure TfrmTransito.zAvisosEmbarqueAfterScroll(DataSet: TDataSet);
begin
  if IsOpen= True then
  begin
...
procedure TfrmTransito.zSalidaInsumoAfterScroll(DataSet: TDataSet);
begin
  if IsOpen then
  begin
...
procedure TfrmTransito.EnterControl(Sender: TObject);
begin
...
procedure TfrmTransito.SalidaControl(Sender: TObject);
begin
    if (sender is tcxDBTextEdit) then
...
procedure TfrmTransito.GlobalKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
