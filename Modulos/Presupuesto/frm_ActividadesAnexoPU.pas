...
uses
  ...
  DBCtrls, ComCtrls, Mask, Utilerias, StrUtils, Menus, Buttons, unitGenerales,
  ExtCtrls, frxClass, frxDBSet, jpeg, RXDBCtrl, DateUtils,
  math, ExtDlgs, Sockets, RxLookup, ZAbstractRODataset,
  ZAbstractDataset, ZDataset, rxCurrEdit, rxToolEdit, MasUtilerias,
  Tabs,
  DBGrids, TeEngine, Series, TeeProcs, Chart, UnitTBotonesPermisos,
  RxMemDS, ComObj, OleServer, ExcelXP, unitexcepciones,
  unitactivapop,
  UFunctionsGHH, cxGraphics, cxControls,
  ...
  dxSkinOffice2013LightGray, cxStyles, dxSkinscxPCPainter,
  cxCustomData, cxFilter, cxData, cxDataStorage, cxNavigator, cxDBData,
  cxGridCustomTableView, cxGridTableView, cxGridDBTableView, cxGridLevel,
  cxClasses, cxGridCustomView, cxGrid, cxGroupBox, dxLayoutContainer,
  dxLayoutControl, cxSplitter, cxCurrencyEdit, cxProgressBar, cxLabel,
  dxSkinOffice2016Colorful, dxSkinOffice2016Dark, dxSkinVisualStudio2013Blue,
  dxSkinVisualStudio2013Dark, dxSkinVisualStudio2013Light, Excel2000;
...
begin
   ...
   if connection.uconfiguracion.FieldByName('lOrdenaItem').AsString = 'No' then
   begin
...
//  frmBarra1.btnCancel.Click;
  ...
  if connection.uconfiguracion.FieldValues['sExplosion'] = 'Recursos por Concepto/Partida' then
...
//              ActividadesxAnexo.FieldValues['iItemOrden'] := sItemOrdenAnterior + sFnBuscaItem(global_convenio, ActividadesxAnexo.FieldValues['sNumeroActividad'],
//               ExtraePaquete,
//               sItemOrdenAnterior,
...
//      NxPCAnexo.ActivePageIndex := 0;
      ...
      try
        ...
        begin
          {Ahora llamamos la funcion que verifica si se puede eliminar la partida..}
//          if BuscaAntesEliminar_datos(2,'actividadesxanexo', 'sContrato', '', 'sWbs', '', global_contrato, '', ActividadesxAnexo.FieldValues['sWbs'], '',
//            'la Partida '+ActividadesxAnexo.FieldValues['sNumeroActividad'], True, progreso) then
//               exit;
        end
        else
        begin
            {Ahora llamamos la funcion que verifica si se puede eliminar el Paquete..}
//            if BuscaAntesEliminar_datos(2,'actividadesxanexo', 'sContrato', '', 'sWbs', '', global_contrato, '', ActividadesxAnexo.FieldValues['sWbs']+'.%', '',
//               'el Paquete '+ActividadesxAnexo.FieldValues['sNumeroActividad'], True, progreso) then
//                  exit;
        end;
...
//  Progreso.Position := 1;
//
//  Progreso.Properties.Max := Actividadesxanexo.RecordCount + 1;
  try
...
//                 exit;
        ...
        Kardex('Conceptos Generales', 'Borra Registro Anexo', ActividadesxAnexo.FieldValues['sNumeroActividad'], ActividadesxAnexo.FieldValues['sTipoActividad'], '', '', '');
      except
        on e: exception do begin
          UnitExcepciones.manejarExcep(E.Message, E.ClassName, 'Conceptos Generales/Partidas de Anexo', 'Al eliminar registro', 0);
        end;
...
begin
  ...
  begin
    ...
    Connection.QryBusca.Filtered := False;
    connection.qryBusca.SQL.Clear;
    If Connection.uconfiguracion.fieldValues['sAlmcon'] = 'CONTRATOS' Then
     begin
...
begin
  ...
  try
    ...
    if TmpName='' then begin
      ...
      fs := Connection.ucontrato.CreateBlobStream(Connection.ucontrato.FieldByName('bImagen'), bmRead);
