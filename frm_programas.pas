...
uses
  ...
  RXDBCtrl, ZAbstractRODataset, ZAbstractDataset, ZDataset, UnitExcepciones, unittbotonespermisos, UnitValidaTexto,
  ExtCtrls,  cxGraphics, cxControls, cxLookAndFeels,
  ...
  unitMetodos, dxSkinOffice2016Colorful, dxSkinOffice2016Dark,
  dxSkinVisualStudio2013Blue, dxSkinVisualStudio2013Dark,
  dxSkinVisualStudio2013Light, cxContainer, cxTextEdit, cxDBEdit, cxMaskEdit,
  cxDropDownEdit, cxLookupEdit, cxDBLookupEdit, cxDBLookupComboBox, cxButtons,
  MemDS, DBAccess, Uni;
...
type
  TfrmProgramas = class(TForm)
    ...
    tsIdPrograma: TcxDBTextEdit;
    tsDescripcion: TcxDBTextEdit;
    tsdbModulos: TcxDBLookupComboBox;
    btnModulos: TcxButton;
    cbidioma: TcxDBLookupComboBox;
    dbedtModulo: TcxDBTextEdit;
    zqryUnionGrid: TUniQuery;
    Programastabla: TUniQuery;
    zqryModulos: TUniQuery;
    zqIdiomas: TUniQuery;
    ...
    ProgramastablaidModulo: TIntegerField;
    ProgramastablasIdIdioma: TStringField;
    ProgramastablascomModulos: TStringField;
...
begin
   ...
   begin
     ...
     Connection.QryUBusca.SQL.Add('Select sDescripcion From idiomas Where sDescripcion = :Idioma');
     Connection.QryUBusca.Params.ParamByName('Idioma').DataType := ftString;
     Connection.QryUBusca.Params.ParamByName('Idioma').Value    := cbSelecIdioma.Text;
    ...
    begin
      if programastabla.Locate('sIdPrograma', zqryUnionGrid.FieldByName('sIdPrograma').AsString,[]) then
       //programastabla.Delete ;
        ...
        Connection.QryUBusca.Active := False ;
        Connection.QryUBusca.SQL.Clear ;
        Connection.QryUBusca.SQL.Add('Select sIdPrograma from gruposxprograma Where sIdPrograma =:programa');
        Connection.QryUBusca.Params.ParamByName('Programa').DataType := ftString ;
        Connection.QryUBusca.Params.ParamByName('Programa').Value    := Programastabla.FieldValues['sIdPrograma'] ;
        Connection.QryUBusca.Open ;
        ...
        Else Begin
          Connection.QryUBusca.Active := False ;
          Connection.QryUBusca.SQL.Clear ;
          Connection.QryUBusca.SQL.Add('Select sIdPrograma from usuariosxprograma Where sIdPrograma =:programa');
          Connection.QryUBusca.Params.ParamByName('Programa').DataType := ftString ;
          Connection.QryUBusca.Params.ParamByName('Programa').Value    := Programastabla.FieldValues['sIdPrograma'] ;
          Connection.QryUBusca.Open ;
          if Connection.QryUBusca.RecordCount > 0 Then
...
begin
  cbidioma.Style.Color := global_color_EntradaERP
end;

procedure TfrmProgramas.cbidiomaExit(Sender: TObject);
begin
  cbidioma.Style.Color := global_color_salidaPU
end;
...
begin
  ...
  programastabla.Params.ParamByName('Idioma').Value := Connection.QryUBusca.FieldValues['sIdIdioma'];
  programastabla.Open;
...
begin
    tsIdPrograma.Style.Color := global_color_EntradaERP
end;

procedure TfrmProgramas.tsIdProgramaExit(Sender: TObject);
begin
    tsIdPrograma.Style.Color := global_color_salidaPU
...
begin
    tsDescripcion.Style.Color := global_color_salidaPU
end;
...
begin
  ...
  if sOldPrograma <> Programastabla.FieldByName('sIdPrograma').AsString  then begin//el ID cambio, actualizar la tabla gruposxprograma
    ...
    try
      ...
      connection.zUCommand.SQL.Add(sUpdate);
      connection.zUCommand.ParamByName('programa').Value := Programastabla.FieldByName('sIdPrograma').AsString;
      connection.zUCommand.ParamByName('oldPrograma').Value := sOldPrograma;
